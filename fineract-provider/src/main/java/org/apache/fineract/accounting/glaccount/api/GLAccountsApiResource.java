...
public class GLAccountsApiResource {
    ...
    private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList("id", "name", "parentId", "glCode",
            ...
            "allowedIncomeTagOptions", "allowedExpensesTagOptions", "creditAccounts", "debitAccounts", "glBalance"));
    ...
    public GLAccountsApiResource(final PlatformSecurityContext context, final GLAccountReadPlatformService glAccountReadPlatformService,
            ...
            final CodeValueReadPlatformService codeValueReadPlatformService, final GLAccountRepository glAccountRepository) {
    ...
    public String retrieveAllAccounts(@Context final UriInfo uriInfo, @QueryParam("type") final Integer type,
            ...
            @QueryParam("manualCharge") final Boolean manualCharge, @QueryParam("fetchRunningBalance") final boolean runningBalance) {
        ...
        final List<GLAccountData> glAccountDatas = this.glAccountReadPlatformService.retrieveAllGLAccounts(type, searchParam, usage,
                manualEntriesAllowed, disabled, manualCharge, associationParametersData);
        
        HashMap parentBal = new  HashMap<Long,BigDecimal>();
		
        for(GLAccountData glAccount : glAccountDatas)
        {
        	BigDecimal parentGlBalance =BigDecimal.ZERO;
        	BigDecimal glBalance =BigDecimal.ZERO;
        	
        	if(glAccount.getParentId()!=null)
        	{
        		glBalance = this.glAccountReadPlatformService.retrieveClosingBalanceOfGLAccount(glAccount.getId());
	        	GLAccount glAcc = this.glAccountRepository.findOne(glAccount.getParentId());
        	
        	if(glAcc != null)
        	{
	        	for(GLAccountData parentGlAccount : glAccountDatas)
	        	{
	        		
	        		if(parentGlAccount.getId().equals(glAcc.getId()))
	        		{
	        			parentGlBalance = parentGlBalance.add(glBalance);
	        			if(parentBal.containsKey(glAcc.getId()))
	        			{
	        				parentGlBalance = (BigDecimal) parentBal.get(glAcc.getId());
	        				parentGlBalance = parentGlBalance.add(glBalance);
	        				parentBal.replace(glAcc.getId(), parentGlBalance);
	        			}
	        		else	
	        		  parentBal.put(glAcc.getId(), parentGlBalance);
	        		  parentGlAccount.setGlBalance(parentGlBalance);
	        		}
	        	}
        	}
        		...
        		glBalance = this.glAccountReadPlatformService.retrieveClosingBalanceOfGLAccount(glAccount.getId());
        	glAccount.setGlBalance(glBalance);
        }
