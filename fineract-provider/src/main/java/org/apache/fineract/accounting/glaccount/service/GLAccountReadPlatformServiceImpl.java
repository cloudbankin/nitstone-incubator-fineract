...
public class GLAccountReadPlatformServiceImpl implements GLAccountReadPlatformService {
    ...
    public List<GLAccountData> retrieveAllGLAccounts(final Integer accountClassification, final String searchParam, final Integer usage,
            final Boolean manualTransactionsAllowed, final Boolean disabled, final Boolean manualCharge, JournalEntryAssociationParametersData associationParametersData) {
        ...
        if ((accountClassification != null) || StringUtils.isNotBlank(searchParam) || (usage != null)
                || (manualTransactionsAllowed != null) || (disabled != null) || (manualCharge != null)) {
            filtersPresent = true;
            sql += " where";
        }
        ...
        if (filtersPresent) {
            ...
            if (manualCharge != null) {
                ...
                if (manualCharge) {
                    sql += " classification_enum = 4";
                } else {
                    sql += " classification_enum IN (1,2,3,4,5)";
                }
                firstWhereConditionAdded = true;
            }
        }
    ...
    public List<GLAccountData> retrieveAllEnabledDetailGLAccounts(final GLAccountType accountType) {
        return retrieveAllGLAccounts(accountType.getValue(), null, GLAccountUsage.DETAIL.getValue(), null, false, false,
                new JournalEntryAssociationParametersData());
    }

    @Override
    public List<GLAccountData> retrieveAllEnabledDetailGLAccounts() {
        return retrieveAllGLAccounts(null, null, GLAccountUsage.DETAIL.getValue(), null, false, false, new JournalEntryAssociationParametersData());
    }
    ...
    public List<GLAccountData> retrieveAllEnabledHeaderGLAccounts(final GLAccountType accountType) {
        return retrieveAllGLAccounts(accountType.getValue(), null, GLAccountUsage.HEADER.getValue(), null, false, false,
                new JournalEntryAssociationParametersData());
    }
    ...
    @Override
    public BigDecimal retrieveClosingBalanceOfGLAccount(Long glAccountId) {
    	 final GLAccountClosingBalanceMapper rm = new GLAccountClosingBalanceMapper();
    	 try {
    		final String sql = "select " + rm.schema();

    		return this.jdbcTemplate.queryForObject(sql, rm, new Object[] {glAccountId });
    	 }catch (final EmptyResultDataAccessException dve) {
        	/*Throwable throwable = ExceptionUtils.getRootCause(dve.getCause()) ;
        	  throw new PlatformDataIntegrityException("error.msg.gl.no.records", "No GL Records found for the Account", "No Records",
                      "No Records");*/
    ...
    private static final class GLAccountClosingBalanceMapper implements RowMapper<BigDecimal> {

        

        @Override
        public BigDecimal mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
        ...
        public String schema() {
            return "je.account_id acid2, \n" + 
            		"if(aga1.classification_enum in (1,5), \n" + 
            		"(sum(if(je.type_enum=2,ifnull(je.amount,0),0))- sum(if(je.type_enum=1,ifnull(je.amount,0),0))), \n" + 
            		"(sum(if(je.type_enum=1,ifnull(je.amount,0),0))- sum(if(je.type_enum=2,ifnull(je.amount,0),0)))) closingBalance \n" + 
