...
public class JournalEntriesApiResource {
    ...
    @GET
    @Path("/{accountId}/{officeId}/{transactionDate}/{hierarchy}")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String getClosingBalanceForDebitOrCreditAccount(@PathParam("accountId") final Long  accountId, @PathParam("officeId") final Long officeId, @PathParam("transactionDate") final Date transactionDate,
    		@PathParam("hierarchy") String  hierarchy, @Context final UriInfo uriInfo) {
        // sum of all debits must be = sum of all credits
       if(hierarchy.equals(".ab."))
       {
    	   hierarchy = ".";
       }
     
        	BigDecimal amount = BigDecimal.ZERO;
        	amount = this.journalEntryReadPlatformService.retrieveClosingBalanceForCredit(accountId, officeId, transactionDate, hierarchy);
        	GLAccount glAccount = this.gLAccountRepository.findOne(accountId);
        	
        	ClosingBalanceData balance = new ClosingBalanceData(amount, glAccount.getDescription());
