...
import org.apache.fineract.accounting.journalentry.data.ClosingBalanceData;
...
import org.apache.fineract.portfolio.loanaccount.jointBorrower.data.JointBorrowerData;
import org.apache.fineract.portfolio.loanaccount.jointBorrower.service.JointBorrowerReadPlatformService;
...
public class LoansApiResource {
    ...
    private final Set<String> LOAN_DATA_PARAMETERS = new HashSet<>(Arrays.asList("id", "accountNo", "status", "externalId", "clientId",
            ...
            "totalFeeChargesAtDisbursement", "summary", "repaymentSchedule", "transactions", "charges", "manualCharges", "collateral", "guarantors",
    ...
    private static final Set<String> RESPONSE_DATA_PARAMETERS_CLOSING_BALANCE = new HashSet<>(Arrays.asList("amount", "description"));
    ...
    private final JointBorrowerReadPlatformService jointBorrowerReadPlatformService;
    ...
    public LoansApiResource(final PlatformSecurityContext context, final LoanReadPlatformService loanReadPlatformService,
            ...
            final GLAccountRepository gLAccountRepository, final DefaultToApiJsonSerializer<Object> apiJsonSerializerService,
            final ProductToGLAccountMappingRepository accountMappingRepository, final JointBorrowerReadPlatformService jointBorrowerReadPlatformService) {
        ...
        this.jointBorrowerReadPlatformService = jointBorrowerReadPlatformService;
    }
    ...
    public String retrieveLoan(@PathParam("loanId") final Long loanId,
        ...
        Collection<LoanManualChargeData> manualCharges = null;
        Collection<GuarantorData> guarantors = null;
        Collection<JointBorrowerData> borrowerData = null;
        ...
        if (!associationParameters.isEmpty()) {
            ...
            if (associationParameters.contains("all")) {
                associationParameters.addAll(Arrays.asList("repaymentSchedule", "futureSchedule", "originalSchedule", "transactions",
                        "charges", "guarantors", "collateral", "notes", "linkedAccount", "multiDisburseDetails","jointBorrowers", "manualCharges"));
            }
            ...
            if (associationParameters.contains("jointBorrowers")) {
                mandatoryResponseParameters.add("jointBorrowers");
                borrowerData = this.jointBorrowerReadPlatformService.retrieveGuarantorsForLoan(loanId);
                if (CollectionUtils.isEmpty(borrowerData)) {
                	borrowerData = null;
                }
            }
            ...
            if (associationParameters.contains("manualCharges")) {
                mandatoryResponseParameters.add("manualCharges");
                manualCharges = this.loanChargeReadPlatformService.retrieveLoanManualCharges(loanId);
                if (CollectionUtils.isEmpty(manualCharges)) {
                	manualCharges = null;
                }
            }
        ...
        final LoanAccountData loanAccount = LoanAccountData.associationsAndTemplate(loanBasicDetails, repaymentSchedule, loanRepayments,
                charges, collateral, guarantors,borrowerData, meeting, productOptions, loanTermFrequencyTypeOptions, repaymentFrequencyTypeOptions,
        ...
        loanAccount.setManualCharges(manualCharges);
    ...
    @GET
    @Path("/getAccountDesc/{paymentId}/{loanId}")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String getGLAccountDescription(@PathParam("paymentId") final Long  paymentId, @PathParam("loanId") final Long  loanId, @Context final UriInfo uriInfo) {
        	...
        	final ProductToGLAccountMapping paymentChannelSpecificAccountMapping = this.accountMappingRepository
                    .findByProductIdAndProductTypeAndFinancialAccountTypeAndPaymentTypeId(loan.getLoanProductId(),
                            PortfolioProductType.LOAN.getValue(), 1, paymentId);
        	
        	String description;
        	
        	if(paymentChannelSpecificAccountMapping != null)
        		description = paymentChannelSpecificAccountMapping.getGlAccount().getDescription();
        	
        	else
        		description = "";
        	
        	ClosingBalanceData balance = new ClosingBalanceData(amount, description);
        	
        	final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        	return this.apiJsonSerializerService.serialize(settings, balance,  RESPONSE_DATA_PARAMETERS_CLOSING_BALANCE);
