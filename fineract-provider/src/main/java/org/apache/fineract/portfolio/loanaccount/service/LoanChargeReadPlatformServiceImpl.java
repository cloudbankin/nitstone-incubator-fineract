...
public class LoanChargeReadPlatformServiceImpl implements LoanChargeReadPlatformService {
    ...
    private static final class LoanChargeMapper implements RowMapper<LoanChargeData> {
        ...
        public String schema() {
            return "lc.id as id, c.id as chargeId, c.name as name, " + "lc.amount as amountDue, "
					...
					+ "lc.charge_payment_mode_enum as chargePaymentMode, " + "lc.is_paid_derived as paid, "
					...
					+ "oc.internationalized_name_code as currencyNameCode, lcad.fees_component, lcad.tax_component, lcad.tax_component_type from m_charge c "
					...
					+ " left join hab_loan_charge_additional_details lcad on lcad.loan_charge_id = lc.id ";
        }
        ...
        public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
         ...
         // Habile changes for tax
         			final BigDecimal feesComponent = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "fees_component");
         			final BigDecimal taxComponent = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "tax_component");
         			/*
         			 * BigDecimal CGSTComponent = BigDecimal.ZERO; BigDecimal SGSTComponent =
         			 * BigDecimal.ZERO; BigDecimal IGSTComponent = BigDecimal.ZERO;
         			 * if(rs.getString("tax_component_type").equalsIgnoreCase("Intra State")) {
         			 * CGSTComponent = taxComponent.divide(new BigDecimal(2)); SGSTComponent =
         			 * taxComponent.subtract(CGSTComponent); }else
         			 * if(rs.getString("tax_component_type").equalsIgnoreCase("Inter State")){
         			 * IGSTComponent = taxComponent; }
         			 */
         			// Habile changes end

         			LoanChargeData loanChargeData = new LoanChargeData(id, chargeId, name, currency, amount, amountPaid,
         					amountWaived, amountWrittenOff, amountOutstanding, chargeTimeType, dueAsOfDate,
         					chargeCalculationType, percentageOf, amountPercentageAppliedTo, penalty, paymentMode, paid, waived,
         					null, minCap, maxCap, amountOrPercentage, null);
         			loanChargeData.setFeesComponent(feesComponent);
         			loanChargeData.setTaxComponent(taxComponent);
         			/*
         			 * loanChargeData.setCGSTComponent(CGSTComponent);
         			 * loanChargeData.setSGSTComponent(SGSTComponent);
         			 * loanChargeData.setIGSTComponent(IGSTComponent);
         			 */
	...
	public Collection<FinabileLoanChargeTaxData> retrieveLoanChargeTaxComponents(final Long loanChargeId) {
		...
		final LoanChargeTaxMapper rm = new LoanChargeTaxMapper();
	...
	private static final class LoanChargeTaxMapper implements RowMapper<FinabileLoanChargeTaxData> {

		public String schema() {
			return " lctd.loan_charge_id, lctd.tax_component_name, lctd.tax_component_amount "
					+ " from hab_loan_charge_tax_details lctd where lctd.loan_charge_id = ? ";
		}

		@Override
		public FinabileLoanChargeTaxData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
				throws SQLException {

			final Long loanChargeId = rs.getLong("loan_charge_id");
			final String taxComponentName = rs.getString("tax_component_name");
			final BigDecimal taxComponentAmount = rs.getBigDecimal("tax_component_amount");

			return new FinabileLoanChargeTaxData(loanChargeId, taxComponentName, taxComponentAmount);
    ...
    @Override
    public Collection<LoanManualChargeData> retrieveLoanManualCharges(final Long loanId) {
        final LoanManualChargeMapper rm = new LoanManualChargeMapper();
        final String sql = "select " + rm.schema()
                + "where lmc.loan_id= ? and lmc.is_active = 1";
        return this.jdbcTemplate.query(sql, rm, new Object[] { loanId});
    }

    private static final class LoanManualChargeMapper implements RowMapper<LoanManualChargeData> {

        public String schema() {
            return "lmc.id as id, lmc.loan_id as loanId, " + "glAcc.name as accountName, "
                    + "lmc.add_charge_date as chargeDate, " + "lmc.amount as amount, " + 
                    "lmc.amount_paid_derived as amountPaid, " + "lmc.description as description, " +
                    "lmc.is_paid_derived as paid " + "from  m_loan_manual_charge lmc " + 
                    "join acc_gl_account glAcc on glAcc.id = lmc.account_id ";
        }

        @Override
        public LoanManualChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final boolean isPaid = rs.getBoolean("paid");
           

            return new LoanManualChargeData(id, loanId, accountName, amount, amountPaid, isPaid, desc, chargeDate);
        }
    }
