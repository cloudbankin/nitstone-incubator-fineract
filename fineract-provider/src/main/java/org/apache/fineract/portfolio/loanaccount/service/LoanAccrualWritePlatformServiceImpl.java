...
public class LoanAccrualWritePlatformServiceImpl implements LoanAccrualWritePlatformService {
    ...
    private void addAccrualAccounting(LoanScheduleAccrualData scheduleAccrualData, BigDecimal amount, BigDecimal interestportion,
    	...
    	final List<Map<String, Object>> transactionMapList= new ArrayList<>();
    	if(interestportion != null && !interestportion.equals(BigDecimal.ZERO)) {
        ...
        this.jdbcTemplate.update(transactionSql, scheduleAccrualData.getLoanId(), scheduleAccrualData.getOfficeId(),
                LoanTransactionType.ACCRUAL.getValue(), accruedTill.toDate(), interestportion, interestportion, fees, penality,
                DateUtils.getDateOfTenant());
        @SuppressWarnings("deprecation")
        final Long transactonId = this.jdbcTemplate.queryForLong("SELECT LAST_INSERT_ID()");
        
        Map<String, Object> transactionMap = toMapData(transactonId, interestportion, interestportion, fees, penality,
                scheduleAccrualData, accruedTill);
        transactionMapList.add(transactionMap);
    	}
    	if(penaltyportion != null&& !penaltyportion.equals(BigDecimal.ZERO)) {
    		BigDecimal interest= BigDecimal.ZERO;BigDecimal fees= BigDecimal.ZERO;
        String transactionSql = "INSERT INTO m_loan_transaction  (loan_id,office_id,is_reversed,transaction_type_enum,transaction_date,amount,interest_portion_derived,"
                + "fee_charges_portion_derived,penalty_charges_portion_derived, submitted_on_date) VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?)";
        this.jdbcTemplate.update(transactionSql, scheduleAccrualData.getLoanId(), scheduleAccrualData.getOfficeId(),
                LoanTransactionType.ACCRUAL.getValue(), accruedTill.toDate(), penaltyportion, interest, fees, penaltyportion,
                DateUtils.getDateOfTenant());
        ...
        Map<String, Object> transactionMap = toMapData(transactonId, penaltyportion, interest, fees, penaltyportion,
                scheduleAccrualData, accruedTill);
        transactionMapList.add(transactionMap);
    	}
    	if(feeportion != null && !feeportion.equals(BigDecimal.ZERO)) {
    		BigDecimal interest= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
        String transactionSql = "INSERT INTO m_loan_transaction  (loan_id,office_id,is_reversed,transaction_type_enum,transaction_date,amount,interest_portion_derived,"
                + "fee_charges_portion_derived,penalty_charges_portion_derived, submitted_on_date) VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?)";
        this.jdbcTemplate.update(transactionSql, scheduleAccrualData.getLoanId(), scheduleAccrualData.getOfficeId(),
                LoanTransactionType.ACCRUAL.getValue(), accruedTill.toDate(), feeportion, interest, feeportion, penality,
                DateUtils.getDateOfTenant());
        ...
        Map<String, Object> transactionMap = toMapData(transactonId, feeportion, interest, feeportion, penality,
                scheduleAccrualData, accruedTill);
        transactionMapList.add(transactionMap);
    	}
        /*@SuppressWarnings("deprecation")
        ...
        final Map<String, Object> accountingBridgeData = deriveAccountingBridgeDataAccrual(scheduleAccrualData, transactionMapList);
    ...
    public Map<String, Object> deriveAccountingBridgeDataAccrual(final LoanScheduleAccrualData loanScheduleAccrualData,
            final List<Map<String, Object>> transactionMapList) {
        ...
        final List<Map<String, Object>> newLoanTransactions = new ArrayList<>();
        for(Map<String, Object> transactionMap:transactionMapList) {
        	 newLoanTransactions.add(transactionMap);
        }
    ...
    private void postJournalEntries(final Loan loan, final List<Long> existingTransactionIds,
        ...
        final String desc = " for the loan " + loan.getAccountNumber() +" of the client " + loan.getClient().getDisplayName();
        this.journalEntryWritePlatformService.createJournalEntriesForLoan(accountingBridgeData, desc);
       // this.journalEntryWritePlatformService.createJournalEntriesForLoan(accountingBridgeData, null);
    }
    
    @Override
    @Transactional
    public void addIncomeAndAccrualTransactionsPrepayment(Long loanId, LocalDate transactionDate, BigDecimal interestdue) throws LoanNotFoundException {
        if (loanId != null) {
            Loan loan = this.loanRepositoryWrapper.findOneWithNotFoundDetection(loanId, true);
            if (loan == null) { throw new LoanNotFoundException(loanId); }
            final List<Long> existingTransactionIds = new ArrayList<>();
            ...
            existingTransactionIds.addAll(loan.findExistingTransactionIds());
            existingReversedTransactionIds.addAll(loan.findExistingReversedTransactionIds());
            loan.processIncomeTransactionsInPrepayment(this.userRepository.fetchSystemUser() , transactionDate,interestdue);
            this.loanRepositoryWrapper.saveAndFlush(loan);
            postJournalEntries(loan, existingTransactionIds, existingReversedTransactionIds);
        }
    }
    
    private void addIncomePostAccounting(LoanTransaction data, Loan loan,  LocalDate transactionDate) throws DataAccessException {
        String transactionSql = "INSERT INTO m_loan_transaction  (loan_id,office_id,is_reversed,transaction_type_enum,transaction_date,amount,interest_portion_derived,"
                + "fee_charges_portion_derived,penalty_charges_portion_derived, submitted_on_date,outstanding_loan_balance_derived) VALUES "
                + "(?, ?, 0, ?, ?, ?, ?, ?, ?, ?,?)";
        ...
        try {
            ...
            final BigDecimal transAmount = data.getAmount(loan.getCurrency()).getAmount();
            
        	PreparedStatement ps = conn.prepareStatement(transactionSql);
			ps.setLong(1, loan.getId());
			ps.setLong(2, loan.getOfficeId());
			ps.setInt(3,  LoanTransactionType.INCOME_POSTING.getValue());
			ps.setDate(4, new java.sql.Date(transactionDate.toDate().getTime()));
			ps.setBigDecimal(5, transAmount);
			ps.setBigDecimal(6, data.getInterestPortion(loan.getCurrency()).getAmount());
			ps.setBigDecimal(7, data.getFeeChargesPortion(loan.getCurrency()).getAmount());
			ps.setBigDecimal(8,data.getPenaltyChargesPortion(loan.getCurrency()).getAmount());
			ps.setDate(9,  new java.sql.Date(DateUtils.getDateOfTenant().getTime()));
			ps.setBigDecimal(10,  data.getOutstandingLoanBalance());
        ...
        /*
        JdbcTemplate jdbcTemplate1 = new JdbcTemplate(this.dataSource);
        
       int rows = jdbcTemplate1.update(transactionSql, loan.getId(), loan.getOfficeId(),
                LoanTransactionType.INCOME_POSTING.getValue(), transactionDate.toDate(), data.getAmount(loan.getCurrency()).getAmount(), data.getInterestPortion(loan.getCurrency()).getAmount(),
                data.getFeeChargesPortion(loan.getCurrency()).getAmount(), data.getPenaltyChargesPortion(loan.getCurrency()).getAmount(),
                DateUtils.getDateOfTenant(), data.getOutstandingLoanBalance());
       ...
       System.out.println("No of rows inserted is"+ rows);
        ...
        //Map<LoanChargeData, BigDecimal> applicableCharges = loan.scheduleAccrualData.getApplicableCharges();
        /*String chargespaidSql = "INSERT INTO m_loan_charge_paid_by (loan_transaction_id, loan_charge_id, amount,installment_number) VALUES (?,?,?,?)";
        for (Map.Entry<LoanChargeData, BigDecimal> entry : applicableCharges.entrySet()) {
            LoanChargeData chargeData = entry.getKey();
            this.jdbcTemplate.update(chargespaidSql, transactonId, chargeData.getId(), entry.getValue(),
