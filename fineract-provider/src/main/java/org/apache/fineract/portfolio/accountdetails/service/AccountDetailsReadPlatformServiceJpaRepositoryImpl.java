...
import org.apache.fineract.portfolio.loanaccount.jointBorrower.data.JointBorrowerLoanData;
...
public class AccountDetailsReadPlatformServiceJpaRepositoryImpl implements AccountDetailsReadPlatformService {
    ...
    public AccountSummaryCollectionData retrieveClientAccountDetails(final Long clientId) {
        ...
        final List<LoanAccuralInterestData> AccuralLoanInterest = retrieveLoanAccural(new Object[] { clientId });
        for(final LoanAccountSummaryData loanaccount : loanAccounts) {
        	BigDecimal processFee =  retrieveLoanProcessFeeAmount(new Object[] { loanaccount.getId() });
        	if(processFee == null) {
        		processFee = BigDecimal.ZERO;
        	}
        	BigDecimal amountPaid=BigDecimal.ZERO;
        	BigDecimal SumOfLoan = loanaccount.getLoanBalance();
    		if(loanaccount.getAmountPaid()!= null){
    			amountPaid = loanaccount.getAmountPaid();
    		}
        	for(final LoanAccuralInterestData interestData:AccuralLoanInterest) {
        		
        		if(loanaccount.getId().equals(interestData.getId())) {
        			SumOfLoan = SumOfLoan.add(interestData.getInterest());        			
        			break;
        		}
        		
        	}
        	SumOfLoan = SumOfLoan.subtract(amountPaid).add(processFee);
        	loanaccount.setLoanBalance(SumOfLoan);
        }
        final List<JointBorrowerLoanData> guarantorData = retriveveGuarantorLoan(new Object[] { clientId });
        for(JointBorrowerLoanData data : guarantorData) {
        	final String whereClause = " where l.id = ?";
        	final LoanAccountSummaryData loanAcc = retrieveLoanAccountDetail(whereClause, new Object[] { data.getLoanId() });
    ...
    private BigDecimal retrieveLoanProcessFeeAmount(final Object[] inputs) {
    	try {
        final String sql = "select IFNULL(ltra11.fee_charges_portion_derived ,0)" + 
        		"from m_loan_transaction ltra11 " + 
        		"where ltra11.loan_id = ? " + 
        		"And ltra11.transaction_type_enum = 5 AND ltra11.is_reversed = 0";
       
        return this.jdbcTemplate.queryForObject(sql, inputs,BigDecimal.class);
    	}
    ...
    private LoanAccountSummaryData retrieveLoanAccountDetail(final String loanwhereClause, final Object[] inputs) {
        final LoanAccountSummaryDataMapper rm = new LoanAccountSummaryDataMapper();
        final String sql = "select " + rm.loanAccountSummarySchema() + loanwhereClause;
        this.columnValidator.validateSqlInjection(rm.loanAccountSummarySchema(), loanwhereClause);
        return this.jdbcTemplate.queryForObject(sql, rm, inputs);
    }
    
    private List<LoanAccuralInterestData> retrieveLoanAccural(final Object[] inputs) {
    ...
    private List<JointBorrowerLoanData> retriveveGuarantorLoan(final Object[] inputs) {
        final LoanGuarantorSummaryDataMapper rm = new LoanGuarantorSummaryDataMapper();
        final String sql = "select " + rm.LoanGuarantorSummaryDataMapper();
        return this.jdbcTemplate.query(sql, rm, inputs);
    }
    ...
    private static final class LoanAccuralSummaryDataMapper implements RowMapper<LoanAccuralInterestData> {

        public String loanAccuralSummarySchema() {
        	final StringBuilder Sql = new StringBuilder(" SUM(ltra1.interest_portion_derived+ltra1.fee_charges_portion_derived + ltra1.penalty_charges_portion_derived) as interest,lm.id as id from m_loan_transaction ltra1 left join m_loan lm on lm.client_id = ? where ltra1.loan_id = lm.id"+
        			" And ltra1.transaction_type_enum = 19 AND ltra1.is_reversed = 0" + 
        			" group by lm.id");
        ...
        @Override
        public LoanAccuralInterestData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
        	  final Long id = JdbcSupport.getLong(rs, "id");
        	  final BigDecimal accuralInterest = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs,"interest");
        	  return new LoanAccuralInterestData(id,accuralInterest);
        }
        
 }
    
    
    private static final class LoanGuarantorSummaryDataMapper implements RowMapper<JointBorrowerLoanData> {

        public String LoanGuarantorSummaryDataMapper() {
        	final StringBuilder Sql = new StringBuilder("g.loan_id as loanId, g.entity_id as clientId from m_jointborrower g where g.entity_id = ? and is_active <> 0");
        	
        	return Sql.toString();
        }
        
        @Override
        public JointBorrowerLoanData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
        	final Long clientId = JdbcSupport.getLong(rs, "clientId");  
        	final Long id = JdbcSupport.getLong(rs, "loanId");
        	return new JointBorrowerLoanData(clientId,id);
        }
    ...
    private static final class LoanAccountSummaryDataMapper implements RowMapper<LoanAccountSummaryData> {
        ...
        public String loanAccountSummarySchema() {
            ...
            accountsSummary
                    ...
                    .append(" l.loan_status_id as statusId, IF(l.loan_subType_enum=0,l.loan_type_enum,l.loan_subType_enum) as loanType,")
                    
                    .append("l.principal_disbursed_derived as originalLoan,")
                    .append("(l.principal_amount+l.manual_charges_charged_derived) as loanBalance,")
                    .append("(l.total_repayment_derived+l.manual_charges_repaid_derived) as amountPaid,")
