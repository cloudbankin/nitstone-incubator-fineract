...
public class ChargesApiResource {
    ...
    public ChargesApiResource(final PlatformSecurityContext context, final ChargeReadPlatformService readPlatformService,
            final DefaultToApiJsonSerializer<ChargeData> toApiJsonSerializer,final ChargeRepository chargeRepository, final ApiRequestParameterHelper apiRequestParameterHelper,
    ...
    public String retrieveCharge(@PathParam("chargeId") final Long chargeId, @Context final UriInfo uriInfo) {
        ...
        /** Habile changes for getting mapped charges */
        final Collection<ChargeData> availableCharges = this.readPlatformService.retrieveAllCharges();
        final Charge oneCharge = this.chargeRepository.findOne(chargeId);
        final Collection<ChargeData> selectedCharges = new ArrayList<>();
        final Set<Charge> charges = oneCharge.getCharges();
        for (final Charge charged : charges) {
        	selectedCharges.add(this.readPlatformService.retrieveCharge(charged.getId()));
        }
		availableCharges.removeAll(selectedCharges);
		charge.setSelectedCharges(selectedCharges);
		charge.setAvailableCharges(availableCharges);
        /** Habile changes end */
    ...
    /** Habile changes Retrieving charge slab values */
	/*@GET
	@Path("slabdetails/{chargeId}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String getSlabDetails(@PathParam("chargeId") final Long chargeId) {
		List<ChargeSlabDetails> listOfSlabs = this.readPlatformService.retriveSlabDetails(chargeId);
		for(ChargeSlabDetails slabValue : listOfSlabs) {
			slabValue.setCharge(null);
		}
		return this.toApiJsonSerializer.serialize(listOfSlabs);
	}
	//retrieving charge additiol details
*/	@GET
	@Path("chargeadditionaldetails/{chargeId}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String getAdditionalChargeDetails(@PathParam("chargeId") final Long chargeId) {
		ChargeAdditionalDetails chargeAdditionalDetails = this.readPlatformService.getChargeAdditionalDetails(chargeId);
		chargeAdditionalDetails.setCharge(null);
		return this.toApiJsonSerializer.serialize(chargeAdditionalDetails);
	}
	/** Habile changes end */
}