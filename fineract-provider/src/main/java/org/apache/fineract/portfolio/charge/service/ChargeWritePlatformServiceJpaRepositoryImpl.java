...
/*import org.apache.fineract.portfolio.charge.data.ChargeSlabDetailDataValidator;*/
...
public class ChargeWritePlatformServiceJpaRepositoryImpl implements ChargeWritePlatformService {
    ...
    /*private final ChargeSlabDetailDataValidator chargeSlabDetailDataValidator;
    ...
    public ChargeWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
            ...
            final TaxGroupRepositoryWrapper taxGroupRepository,/*final ChargeSlabDetailDataValidator chargeSlabDetailDataValidator,final ChargeSlabDetailsRepository chargeSlabDetailsRepository,*/
        ...
        /*this.chargeSlabDetailDataValidator=chargeSlabDetailDataValidator;
    ...
    public CommandProcessingResult createCharge(final JsonCommand command) {
        try {
            ...
            /*
			 * Habile changes start Following code is used to save slab values
			 */
          /*  if (charge != null && charge.getChargeCalculation() == 7) {
					this.chargeSlabDetailDataValidator.validateForSlabAmounts(command.json());
					Date createdDate = new Date();
					Date lastmodifiedDate = null;
					AppUser createdById = this.context.authenticatedUser();
					AppUser lastmodifiedById = null;
					List<ChargeSlabDetails> slabChargeList = ChargeSlabDetails.fromJson(command, charge, createdDate,
							lastmodifiedDate, lastmodifiedById, createdById);
					if (slabChargeList != null && slabChargeList.size() > 0) {
						for (ChargeSlabDetails chargeSlabDetails : slabChargeList) {
							this.chargeSlabDetailsRepository.save(chargeSlabDetails);
						}
					}
				}*/
				
				 //Save additional details for charges
			
				 boolean isEnabledAutoPaid = command.booleanPrimitiveValueOfParameterNamed("isEnabledAutoPaid");
				 boolean isEnabledFeeCalculationBasedOnTenure = command.booleanPrimitiveValueOfParameterNamed("isEnabledFeeCalculationBasedOnTenure");
				 boolean isTaxIncluded = command.booleanPrimitiveValueOfParameterNamed("isTaxIncluded");
				 
				 Integer penaltyWaitPeriod = command.integerValueOfParameterNamed("penaltyWaitPeriod");
				 Integer graceOnPenaltyPosting = command.integerValueOfParameterNamed("graceOnPenaltyPosting");
				 
				 Integer isEnabledAutoPaidInt=isEnabledAutoPaid?1:0;
				 Integer isEnabledFeeCalculationBasedOnTenureInt = isEnabledFeeCalculationBasedOnTenure?1:0;
				 Integer isTaxIncludedInt = isTaxIncluded?1:0;
					...
					AppUser createdById = this.context.authenticatedUser();
					AppUser lastmodifiedById = null;
					ChargeAdditionalDetails chargeAdditionalDetails = ChargeAdditionalDetails.fromJson(charge,
							isEnabledFeeCalculationBasedOnTenureInt, isEnabledAutoPaidInt, isTaxIncludedInt, createdDate, lastmodifiedDate,
							lastmodifiedById, createdById,penaltyWaitPeriod,graceOnPenaltyPosting);
					this.chargeAdditionalDetailsRepository.save(chargeAdditionalDetails);
    ...
    public CommandProcessingResult updateCharge(final Long chargeId, final JsonCommand command) {
        ...
        try {
			 ...
			 */
		//	try {
			/*if (chargeForUpdate != null && chargeForUpdate.getChargeCalculation() == 7) {
				Date createdDate = new Date();
				Date lastmodifiedDate = new Date();
				AppUser createdById = this.context.authenticatedUser();
				AppUser lastmodifiedById = null;
				this.chargeSlabDetailDataValidator.validateForSlabAmounts(command.json());
				// this.chargeSlabDetailsRepository.deleteSlabDetails(chargeId);
				List<ChargeSlabDetails> slabs = this.chargeSlabDetailsRepository.findByChargeId(chargeId);
				if (slabs != null && slabs.size() > 0) {
					for (ChargeSlabDetails slab : slabs) {
						this.chargeSlabDetailsRepository.delete(slab);
					}
				}
				List<ChargeSlabDetails> slabChargeList = ChargeSlabDetails.fromJson(command, chargeForUpdate,
						createdDate, lastmodifiedDate, lastmodifiedById, createdById);
				if (slabChargeList != null && slabChargeList.size() > 0) {
					for (ChargeSlabDetails chargeSlabDetails : slabChargeList) {
						this.chargeSlabDetailsRepository.save(chargeSlabDetails);
					}
				...
				List<ChargeSlabDetails> chargeSlabDetails = this.chargeSlabDetailsRepository.findByChargeId(chargeId);
				if (!chargeSlabDetails.isEmpty()) {
					this.chargeSlabDetailsRepository.deleteSlabDetails(chargeId);
				}
			}*/
				...
				if(chargeForUpdate != null /*&& chargeForUpdate.getChargeCalculation() != 7*/){
					 ChargeAdditionalDetails chargeAdditionalDetails = this.chargeAdditionalDetailsRepository.getChargeAdditionalDetails(chargeForUpdate);
					 if(chargeAdditionalDetails == null) {
						 throw new NotFoundException("Charge additional details not found");
					 }
					 boolean isEnabledAutoPaid = command.booleanPrimitiveValueOfParameterNamed("isEnabledAutoPaid");
					 boolean isEnabledFeeCalculationBasedOnTenure = command.booleanPrimitiveValueOfParameterNamed("isEnabledFeeCalculationBasedOnTenure");
					 boolean isTaxIncluded = command.booleanPrimitiveValueOfParameterNamed("isTaxIncluded");
					 ...
					 Integer graceOnPenaltyPosting = command.integerValueOfParameterNamed("graceOnPenaltyPosting");
						...
						Date lastmodifiedDate = new Date();
						//AppUser createdById = this.context.authenticatedUser();
						AppUser lastmodifiedById = this.context.authenticatedUser();
						chargeAdditionalDetails.setIsEnabledAutoPaid(isEnabledAutoPaidInt);
						chargeAdditionalDetails.setIsEnabledFeeCalculationBasedOnTenure(isEnabledFeeCalculationBasedOnTenureInt);
						chargeAdditionalDetails.setIsTaxIncluded(isTaxIncludedInt);
						chargeAdditionalDetails.setLastmodifiedById(lastmodifiedById);
						chargeAdditionalDetails.setLastmodifiedDate(lastmodifiedDate);
						chargeAdditionalDetails.setPenaltyWaitPeriod(penaltyWaitPeriod);
						chargeAdditionalDetails.setGraceOnPenaltyPosting(graceOnPenaltyPosting);
