...
import org.apache.fineract.portfolio.accountdetails.data.LoanAccuralInterestData;
...
public class LoanReadPlatformServiceImpl implements LoanReadPlatformService {
    ...
    public Long retrieveLoanAccural(Long inputs) {
    	Long Zero=new Long(0);
    	 try {
             this.context.authenticatedUser();
        final LoanAccuralSummaryDataMapper rm = new LoanAccuralSummaryDataMapper();
        final String sql = "select " + rm.loanAccuralSummarySchema();
        return this.jdbcTemplate.queryForLong(sql,inputs);
    	 }catch (final EmptyResultDataAccessException e) {
             return Zero;
         }
    }

    private static final class LoanAccuralSummaryDataMapper{

        public String loanAccuralSummarySchema() {
        	final StringBuilder Sql = new StringBuilder("(SUM(ltra1.interest_portion_derived)+lm.fee_charges_charged_derived+lm.penalty_charges_charged_derived+lm.principal_amount) as interest from m_loan_transaction ltra1 left join m_loan lm on lm.id = ? where ltra1.loan_id = lm.id"+
        			" And ltra1.transaction_type_enum = 10 AND ltra1.is_reversed = 0");
    ...
    @Override
    public LoanTransactionData retrieveLoanPrePaymentTemplateNew(final Long loanId, LocalDate onDate) {
    ...
    public LoanTransactionData retrieveLoanPrePaymentTemplate(final Long loanId, LocalDate onDate) {
        ...
        for(LoanCharge loanCharge: loan.charges()){
        	if(loanCharge.isPrePayment() && !loanCharge.isWaived()){  
        		isPrepaymentCharge=true;
        		if(loanCharge.getDueLocalDate().equals(onDate)){        			
        			addCharge.add(loanCharge);
        		}
				}
        	chargeDate = loanCharge.getDueLocalDate();
        	}
        if(isPrepaymentCharge && addCharge.isEmpty()) {
        ...
        final LoanTransactionEnumData transactionType = LoanEnumerations.transactionType(LoanTransactionType.REPAYMENT);
        final Collection<PaymentTypeData> paymentOptions = this.paymentTypeReadPlatformService.retrieveAllPaymentTypes();
        final BigDecimal outstandingLoanBalance = loan.getSummary().getTotalPrincipalOutstanding();
        final BigDecimal unrecognizedIncomePortion = null;
         LocalDate lastCompondingDate = loan.findLastCompondingDate(onDate);
         Money interestOutstanding = loan.getReceivableInterestIncomePosting(onDate);
        final BigDecimal feeOutstanding = loan.getLoanSummary().getTotalFeeChargesCharged().subtract(loan.getSummary().getTotalFeeChargesRepaid());
        final BigDecimal penaltyOutstanding = loan.getLoanSummary().getTotalPenaltyChargesCharged().subtract(loan.getSummary().getTotalPenaltyChargesRepaid()).subtract(loan.getSummary().getTotalPenaltyChargesWaived());
         Money actualOutanding = Money.of(currency,outstandingLoanBalance.add(feeOutstanding).add(penaltyOutstanding));
        actualOutanding = actualOutanding.plus(interestOutstanding);
        final LoanApplicationTerms loanApplicationTerms = loan.constructLoanApplicationTerms(scheduleGeneratorDTO);
        if(!lastCompondingDate.isEqual(onDate)) {
        	lastCompondingDate=lastCompondingDate.plusDays(1);
        }
        Money interestForPendingDate=calculateInterest(loanApplicationTerms,actualOutanding,lastCompondingDate,onDate);
        interestOutstanding= interestOutstanding.plus(interestForPendingDate);
        final BigDecimal TotalOutstanding = outstandingLoanBalance.add(feeOutstanding).add(penaltyOutstanding).add(interestOutstanding.getAmount());
        BigDecimal adjustedChargeAmount = adjustPrepayInstallmentCharge(loan, onDate);
        return new LoanTransactionData(null, null, null, transactionType, null, currencyData, earliestUnpaidInstallmentDate,
        		TotalOutstanding.add(adjustedChargeAmount), loanRepaymentScheduleInstallment
                        .getPrincipalOutstanding(currency).getAmount(), interestOutstanding
                        .getAmount(), feeOutstanding.subtract(adjustedChargeAmount),
                        penaltyOutstanding, null, unrecognizedIncomePortion,
                paymentOptions, null, null, null, outstandingLoanBalance, false);
    }
    
    
    public Money calculateInterest(LoanApplicationTerms loanApplication ,Money actualOutstanding,LocalDate lastCompondingDate, LocalDate onDate) {
    	final RoundingMode roundingMode = MoneyHelper.getRoundingMode();
        final MathContext mc = new MathContext(8, roundingMode);       
		Money interestDue = loanApplication.calculateDecliningInterestDueForInstallmentBeforeApplyingGracePeriod(this.paymentPeriodsInOneYearCalculator, mc,actualOutstanding,lastCompondingDate, onDate);
		return interestDue;
    }
    
    
    @Override
    public LoanTransactionData retrieveLoanPrePaymentTemplateGetAddCharge(final Long loanId, LocalDate onDate) {
        ...
        BigDecimal adjustedChargeAmount = adjustPrepayInstallmentCharge(loan, onDate);
        return new LoanTransactionData(null, null, null, transactionType, null, currencyData, earliestUnpaidInstallmentDate,
                loanRepaymentScheduleInstallment.getTotalOutstanding(currency).getAmount().add(adjustedChargeAmount), loanRepaymentScheduleInstallment
                        .getPrincipalOutstanding(currency).getAmount(), loanRepaymentScheduleInstallment.getInterestOutstanding(currency)
                        .getAmount(), loanRepaymentScheduleInstallment.getFeeChargesOutstanding(currency).getAmount().subtract(adjustedChargeAmount),
                loanRepaymentScheduleInstallment.getPenaltyChargesOutstanding(currency).getAmount(), null, unrecognizedIncomePortion,
                paymentOptions, null, null, null, outstandingLoanBalance, false);
    ...
    private static final class LoanMapper implements RowMapper<LoanAccountData> {
        ...
        public String loanSchema() {
            return "l.id as id, l.account_no as accountNo, l.external_id as externalId, l.fund_id as fundId, f.name as fundName,"
                    ...
                    + " c.sub_status as clientSubStatus, "
        ...
        public LoanAccountData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final Long subStatusId = JdbcSupport.getLong(rs, "clientSubStatus");
    ...
    private static final class LoanTransactionsMapper implements RowMapper<LoanTransactionData> {
        ...
        public String LoanPaymentsSchema() {
            ...
            return " tr.id as id, tr.transaction_type_enum as transactionType, tr.transaction_date as `date`, tr.amount as total, "
                    ...
                    + " tr.fee_charges_portion_derived as fees, tr.penalty_charges_portion_derived as penalties, tr.manual_charges_portion_derived as manualCharges, "
        ...
        public LoanTransactionData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final BigDecimal manualChargesPortion = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "manualCharges");
            int transactionTypeIdInt = 0;
			if(transactionTypeInt == 19) {	
				if(feeChargesPortion.compareTo(BigDecimal.ZERO) > 0){
		        	 transactionTypeIdInt = 20;
		         }
				else if(penaltyChargesPortion.compareTo(BigDecimal.ZERO) > 0){
					transactionTypeIdInt = 21;
				}
				else {
					transactionTypeIdInt = transactionTypeInt;
				}
            ...
            final LoanTransactionEnumData transactionType = LoanEnumerations.transactionType(transactionTypeIdInt);
            ...
            loanTransactionData.setManualChargesPortion(manualChargesPortion);
    ...
    public Collection<OverdueLoanScheduleData> retrieveAllLoansWithOverdueInstallments(final Long penaltyWaitPeriod,
        ...
        sqlBuilder.append("select ").append(rm.schema()).append(" where DATE_SUB(CURDATE(),INTERVAL ? DAY) > ls.duedate ")
                ...
                .append(" and mc.charge_time_enum = 9 and ml.loan_status_id = 300 and ls.loan_id not in (58) ");
    ...
    public Collection<LoanScheduleAccrualData> retrivePeriodicAccrualData(final LocalDate tillDate) {
        ...
        sqlBuilder
                .append("select ")
                .append(mapper.schema())
                .append(" where  (recaldet.is_compounding_to_be_posted_as_transaction is not null or recaldet.is_compounding_to_be_posted_as_transaction = 0) ")
                .append(" and (((ls.fee_charges_amount <> if(ls.accrual_fee_charges_derived is null,0, ls.accrual_fee_charges_derived))")
                .append(" or (ls.penalty_charges_amount <> if(ls.accrual_penalty_charges_derived is null,0,ls.accrual_penalty_charges_derived))")
                .append(" or (ls.interest_amount <> if(ls.accrual_interest_derived is null,0,ls.accrual_interest_derived)))")
    ...
    public LoanTransactionData retrieveLoanWriteoffTemplate(final Long loanId) {
        ...
        final Loan fetchLoan = this.loanRepositoryWrapper.findOneWithNotFoundDetection(loanId, true);
        final BigDecimal outstandingLoanBalance = null;
        final LoanTransactionEnumData transactionType = LoanEnumerations.transactionType(LoanTransactionType.WRITEOFF);
        final BigDecimal unrecognizedIncomePortion = null;
        final BigDecimal loanBalance = fetchLoan.getSummary().getTotalPrincipalOutstanding().add(fetchLoan.getSummary().getTotalFeeChargesOutstanding())
        		.add(fetchLoan.getSummary().getTotalPenaltyChargesOutstanding());
        LoanTransactionData transactionData = retrieveLoanPrePaymentTemplate(loanId , DateUtils.getLocalDateOfTenant());
        ...
        LoanTransactionData loanTransactionData = new LoanTransactionData(null, null, null, transactionType, null, loan.currency(), DateUtils.getLocalDateOfTenant(),
        		transactionData.getAmount(), null, null, null, null, null, null, null, null, outstandingLoanBalance,
