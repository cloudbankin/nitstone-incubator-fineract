...
public class Loan extends AbstractPersistableCustom<Long> {
    ...
    @Column(name = "loan_subType_enum", nullable = false)
    private Integer loanSubType;
    ...
    @Transient
	private Boolean isLoanExtension = false;
    
    public Boolean getIsLoanExtension() {
		return isLoanExtension;
	}

	public void setIsLoanExtension(Boolean isLoanExtension) {
		this.isLoanExtension = isLoanExtension;
	}
    ...
    /*Habile_Change*/
    public void setLoanSubType() {
    	this.loanSubType = 0;
    }

    public void updateLoanType(Integer type) {
        this.loanSubType =  type;
    }
    ...
    // Habile Arivu Penalty Change 
    
    public void waiveLoanCharge1(final LoanCharge loanCharge, final LoanLifecycleStateMachine loanLifecycleStateMachine,
      ...
      /*  final LoanTransaction waiveLoanChargeTransaction = LoanTransaction.waiveLoanCharge(this, getOffice(), amountWaived,
        ...
        if (!loanCharge.isDueAtDisbursement()) {
            ...
            // reprocess loan schedule based on charge been waived.
            final LoanRepaymentScheduleProcessingWrapper wrapper = new LoanRepaymentScheduleProcessingWrapper();
            wrapper.reprocessForMonthEnd(getCurrency(), getDisbursementDate(), getRepaymentScheduleInstallments(), charges());
       // }
        ...
        if (disbursementID != null) {
            ...
            for (LoanTrancheCharge trancheCharge : trancheCharges) {
                ...
                final LoanCharge loanCharge = LoanCharge.createNewWithoutLoan(chargeDefinition, principal, null, null, null, new LocalDate(
                        expectedDisbursementDate), null, null,null);
                loanCharge.update(this);
    ...
    public LoanScheduleModel regenerateScheduleModel(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
        ...
        final LoanScheduleModel loanSchedule = loanScheduleGenerator.generate(mc, loanApplicationTerms, charges(),
                scheduleGeneratorDTO.getHolidayDetailDTO(), null, this.getLoanProduct());
        return loanSchedule;
    }
	 ...
	 */
	public void handleInstallmentRescheduleTransaction(final LocalDate disbursedOn, final AppUser currentUser,
			final LoanCharge charge, final PaymentDetail paymentDetail, final Money extensionFeeAmount) {
		...
		/***
		 * TODO Vishwas: do we need to be able to pass in payment type details for
		 * repayments at disbursements too?
		 ***/

		final Money totalFeeChargesDueAtDisbursement = extensionFeeAmount;
		/**
		 * all Charges repaid at disbursal is marked as repaid and "APPLY Charge"
		 * transactions are created for all other fees ( which are created during
		 * disbursal but not repaid)
		 **/

		Money disbursentMoney = Money.zero(getCurrency());
		final LoanTransaction chargesPayment = LoanTransaction.repaymentAtInstallmentRescheduled(getOffice(),
				disbursentMoney, paymentDetail, disbursedOn, null, createdDate, currentUser);
		final Integer installmentNumber = null;
		/*
		 * for (final LoanCharge charge : charges()) { //Date actualDisbursementDate =
		 * getActualDisbursementDate(charge); if
		 * ((charge.getCharge().getChargeTimeType() ==
		 * ChargeTimeType.INSTALLMENT_RESCHEDULED.getValue() && !charge.isWaived() &&
		 * charge .isFullyPaid())) { if
		 * (totalFeeChargesDueAtDisbursement.isGreaterThanZero() &&
		...
		if (getExpectedFirstRepaymentOnDate() != null
			...
			throw new InvalidLoanStateTransitionException("disbursal", "cannot.be.after.expected.first.repayment.date",
					errorMessage, disbursedOn, getExpectedFirstRepaymentOnDate());
		}
    ...
    private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(final LoanTransaction loanTransaction,
        ...
        if (reprocess) {
            ...
            changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
                    allNonContraTransactionsPostDisbursement, getCurrency(), getRepaymentScheduleInstallments(), charges());
            LoanWritePlatformServiceJpaRepositoryImpl.prepay=0;
            for (final Map.Entry<Long, LoanTransaction> mapEntry : changedTransactionDetail.getNewTransactionMappings().entrySet()) {
                mapEntry.getValue().updateLoan(this);
            }
            /*
			...
			for (final LoanCharge loanCharge : charges) {
				if (loanCharge.isInstallmentRescheduled() && (loanCharge.getLoanChargeAdditionalDetails() != null
					...
					extensionFeeCharged = extensionFeeCharged.add(loanCharge.getAmount(getCurrency()).getAmount());
					extensionFeePaid = extensionFeePaid.add(loanCharge.getAmountPaid(getCurrency()).getAmount());
				}
			}

			for (LoanRepaymentScheduleInstallment loanRepaymentSchedule : getRepaymentScheduleInstallments()) {
				if (loanRepaymentSchedule.getDueDate().equals(lastRepaymentDate)
						&& loanRepaymentSchedule.getFeeChargesCharged() != null) {
					loanRepaymentSchedule.setFeeChargesCharged(
							loanRepaymentSchedule.getFeeChargesCharged().add(extensionFeeCharged));
    ...
    public List<LoanTransaction> retreiveListOfTransactionsPostDisbursements() {
    ...
    public ChangedTransactionDetail closeAsWrittenOff(final JsonCommand command, final LoanLifecycleStateMachine loanLifecycleStateMachine,
        ...
        if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
            ...
            addLoanTransaction(loanTransaction) ;
            loanRepaymentScheduleTransactionProcessor.handleWriteOffLoan(loanTransaction, loanCurrency(), getRepaymentScheduleInstallments());
    ...
    public LocalDate getCreationOnDate() {
        LocalDate date = null;
        if (this.CreationDate != null) {
            date = new LocalDate(this.CreationDate);
        }
        return date;
    }
    ...
    public LocalDate findLastCompondingDate(final LocalDate tillDate) {
    	 LocalDate compondingDate = LocalDate.now(); 
    	for (final LoanTransaction transaction : this.loanTransactions) {    		
            if (transaction.isNotReversed() && !transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                ...
                if (transaction.isIncomePosting()) {
                	compondingDate = transaction.getTransactionDate();
                }
            }
          
    	}    	
    	
    	  return compondingDate;
    ...
    /**Habile get Total Interest Posting**/
    public Money getReceivableInterestIncomePosting(final LocalDate tillDate) {
        Money receivableInterest = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (transaction.isNotReversed() && !transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate)) {
    ...
    public void validateChargeDateIsOnNonWorkingDay(final LocalDate chargeDate, final WorkingDays workingDays,
            final boolean allowTransactionsOnNonWorkingDay) {
        if (!allowTransactionsOnNonWorkingDay) {
            if (!WorkingDaysUtil.isWorkingDay(workingDays, chargeDate)) {
                final String errorMessage = "Charge Transaction date cannot be on a non working day";
                throw new LoanApplicationDateException("charge.transaction.date.on.non.working.day", errorMessage, chargeDate);
            }
        }
    }

    public void validateChargeDateIsOnHoliday(final LocalDate chargeDate, final boolean allowTransactionsOnHoliday,
            final List<Holiday> holidays) {
        if (!allowTransactionsOnHoliday) {
            if (HolidayUtil.isHoliday(chargeDate, holidays)) {
                final String errorMessage = "Charge Transaction date cannot be on a holiday";
                throw new LoanApplicationDateException("charge.transaction.date.on.holiday", errorMessage, chargeDate);
            }
        }
    }
    ...
    public ChangedTransactionDetail handleRegenerateRepaymentScheduleWithInterestRecalculationwhilePreclose(final ScheduleGeneratorDTO generatorDTO,
            final AppUser currentUser) {
        regenerateRepaymentScheduleWithInterestRecalculationWhilePreclose(generatorDTO, currentUser);
        return processTransactions();
    ...
    public void regenerateRepaymentScheduleWithInterestRecalculation(final ScheduleGeneratorDTO generatorDTO, final AppUser currentUser) {
        ...
        Set<LoanCharge> charges = this.charges();
        for (final LoanCharge loanCharge : charges) {
            if (!loanCharge.isDueAtDisbursement()/* Habile changes for extension fee */
					&& (loanCharge.isPenaltyCharge() || (loanCharge.getLoanChargeAdditionalDetails() != null
    ...
    public void regenerateRepaymentScheduleWithInterestRecalculationWhilePreclose(final ScheduleGeneratorDTO generatorDTO, final AppUser currentUser) {
    ...
    public void updateLoanChargesPaidBy(LoanTransaction accrual, HashMap<String, Object> feeDetails,
            LoanRepaymentScheduleInstallment installment) {
    ...
    public void processIncomeTransactions(AppUser currentUser) {
        if (this.loanInterestRecalculationDetails != null && this.loanInterestRecalculationDetails.isCompoundingToBePostedAsTransaction()) {
            ...
            for (LoanInterestRecalcualtionAdditionalDetails compoundingDetail : compoundingDetails) {
                ...
                List<LoanTransaction> incomeTransactionList = getTransactionListForDate(incomeTransactions, compoundingDetail.getEffectiveDate());
                ...
                addUpdateIncomeAndAccrualTransaction(compoundingDetail, lastCompoundingDate, currentUser, incomeTransaction,
                        accrualTransaction,incomeTransactionList);
    ...
    /** Habile Code 
     * add Income post remaining accural while prepayment
     * @param transactions
     * @param date
     */
    public void processIncomeTransactionsInPrepayment(AppUser currentUser, LocalDate transactionDate,BigDecimal interestdue) {
        if (this.loanInterestRecalculationDetails != null && this.loanInterestRecalculationDetails.isCompoundingToBePostedAsTransaction()) {
            LocalDate lastCompoundingDate = this.getDisbursementDate();
            ...
            List<LoanTransaction> accrualTransactions = retreiveListOfAccrualTransactions();
            for (LoanInterestRecalcualtionAdditionalDetails compoundingDetail : compoundingDetails) {   
                  	if (!compoundingDetail.getEffectiveDate().isBefore(transactionDate) && 
            			!(compoundingDetail.getEffectiveDate().getMonthOfYear()==transactionDate.getMonthOfYear())) {
                    break;
                }
                LoanTransaction incomeTransaction = getTransactionForDate(incomeTransactions, compoundingDetail.getEffectiveDate());
                LoanTransaction accrualTransaction = getTransactionForDate(accrualTransactions, compoundingDetail.getEffectiveDate());
                List<LoanTransaction> incomeTransactionList = getTransactionListForDate(incomeTransactions, transactionDate);
                addUpdateIncomeAndAccrualTransactionForPrepayment(compoundingDetail, lastCompoundingDate, currentUser, incomeTransaction,
                		accrualTransactions,transactionDate,incomeTransactionList,interestdue);
                lastCompoundingDate = compoundingDetail.getEffectiveDate();
            }
            List<LoanRepaymentScheduleInstallment> installments = getRepaymentScheduleInstallments() ;
            LoanRepaymentScheduleInstallment lastInstallment = installments.get(installments.size() - 1);
    ...
    private void addUpdateIncomeAndAccrualTransaction(LoanInterestRecalcualtionAdditionalDetails compoundingDetail,
            ...
            LoanTransaction existingAccrualTransaction,List<LoanTransaction> existingIncomeTransactionList) {
        BigDecimal interest = BigDecimal.ZERO;
        BigDecimal fee = BigDecimal.ZERO;
        BigDecimal penalties = BigDecimal.ZERO;
        BigDecimal compInterest = BigDecimal.ZERO;
        BigDecimal compFee = BigDecimal.ZERO;
        BigDecimal comPenalties = BigDecimal.ZERO;
        BigDecimal existingTransactionAmount = BigDecimal.ZERO;
        List<LoanTransaction> exInterestIncomeTransaction = new ArrayList<>();
        List<LoanTransaction> exFeeIncomeTransaction = new ArrayList<>();
        List<LoanTransaction> exPenalityIncomeTransaction = new ArrayList<>();
        HashMap<String, Object> feeDetails = new HashMap<>();
        for(LoanTransaction transaction : existingIncomeTransactionList) {   
        	compInterest = compInterest.add(transaction.getInterestPortion(getCurrency()).getAmount());        	
        	compFee = compFee.add(transaction.getFeeChargesPortion(getCurrency()).getAmount());
        	comPenalties = comPenalties.add(transaction.getPenaltyChargesPortion(getCurrency()).getAmount());
        	existingTransactionAmount= existingTransactionAmount.add(transaction.getAmount(getCurrency()).getAmount()); 
        	if(!transaction.getInterestPortion(getCurrency()).getAmount().equals(BigDecimal.ZERO)) {
       		 exInterestIncomeTransaction.add(transaction);
        	}
        	if(!transaction.getFeeChargesPortion(getCurrency()).getAmount().equals(BigDecimal.ZERO)) {
       		exFeeIncomeTransaction.add(transaction);
        	}
        	if(!transaction.getPenaltyChargesPortion(getCurrency()).getAmount().equals(BigDecimal.ZERO)) {
       		exPenalityIncomeTransaction.add(transaction);
        	}
        ...
        if (existingIncomeTransaction == null) {
        	if(!interest.equals(BigDecimal.ZERO)) {
        		 BigDecimal fees= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
            LoanTransaction interestTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
                    .toDate(), interest, interest, fees, penality, currentUser);
            addLoanTransaction(interestTransaction);
        	}
        	if(!fee.equals(BigDecimal.ZERO)) {
        		 BigDecimal interests= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
                 LoanTransaction feeTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
                         .toDate(), fee, interests, fee, penality, currentUser);
                 addLoanTransaction(feeTransaction);
                 updateLoanChargesPaidBy(feeTransaction, feeDetails, null);
        	}
        	if(!penalties.equals(BigDecimal.ZERO)) {
       		 BigDecimal interests= BigDecimal.ZERO;BigDecimal fees= BigDecimal.ZERO;
                LoanTransaction penaltiesTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
                        .toDate(), penalties, interests, fees, penalties, currentUser);
                addLoanTransaction(penaltiesTransaction);
                updateLoanChargesPaidBy(penaltiesTransaction, feeDetails, null);
       	}
            BigDecimal fees= BigDecimal.ZERO;
            BigDecimal penality= BigDecimal.ZERO;
           LoanTransaction accrual = LoanTransaction.accrueInterestTransaction(this, this.getOffice(), compoundingDetail.getEffectiveDate(),
                    compoundingDetail.getAmount(), interest, fees, penality, currentUser);
            addLoanTransaction(accrual);
            
            
        } else if (existingTransactionAmount.compareTo(compoundingDetail.getAmount()) != 0) {
        	
            if(!interest.equals(BigDecimal.ZERO) && interest.compareTo(compInterest) != 0) {
            	 for(LoanTransaction transaction : exInterestIncomeTransaction) {   
            		 transaction.reverse();
            	 }
       		 BigDecimal fees= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
       		LoanTransaction interestTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
           ...
           addLoanTransaction(interestTransaction);
            ...
            if(!fee.equals(BigDecimal.ZERO) && fee.compareTo(compFee)!= 0) {
       		 BigDecimal interests= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
       		 for(LoanTransaction transaction : exFeeIncomeTransaction) {   
        		 transaction.reverse();
        	 }
                LoanTransaction feeTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
                        .toDate(), fee, interests, fee, penality, currentUser);
            ...
            if(!penalties.equals(BigDecimal.ZERO) && penalties.compareTo(comPenalties)!= 0) {
            	
          		 BigDecimal interests= BigDecimal.ZERO;BigDecimal fees= BigDecimal.ZERO;
          		for(LoanTransaction transaction : exPenalityIncomeTransaction) {   
              		 transaction.reverse();
              	 }
                   LoanTransaction penaltiesTransaction = LoanTransaction.incomePosting(this, this.getOffice(), compoundingDetail.getEffectiveDate()
                           .toDate(), penalties, interests, fees, penalties, currentUser);
        ...
        */}
        ...
        List<LoanTransaction> tempList = new ArrayList<LoanTransaction>();
        for(LoanTransaction transaction : loanTransactions) {
        	if(transaction.getId()== null && transaction.isIncomePosting()) {
        		tempList.add(transaction);        		
        	}        	        	
        }
        if(!tempList.isEmpty()) {
        	LoanTransaction trans =  Collections.max(tempList, Comparator.comparing(s -> s.getOutstandingLoanBalance()));
	        tempList.remove(trans);
	        tempList.add(0,trans);
	        for(LoanTransaction temp : tempList) {
	        	this.loanTransactions.remove(temp);
	        	this.loanTransactions.add(temp);
	        }
        }
    }
    
    
    public BigDecimal retreiveNonPostedAmountoforPrepayment(LocalDate lastCompoundingDate, LocalDate transactionDate, BigDecimal interestdue) {
        BigDecimal accrual =  BigDecimal.ZERO;
        
        Money totalAccural = getReceivableInterest(transactionDate);   
        
        Money incomePosting = getReceivableInterestIncomePosting(transactionDate);
        
		return incomePosting.minus(interestdue).getAmount();
        
    }
    
    private void addUpdateIncomeAndAccrualTransactionForPrepayment(LoanInterestRecalcualtionAdditionalDetails compoundingDetail,
            LocalDate lastCompoundingDate, AppUser currentUser, LoanTransaction existingIncomeTransaction,
            List<LoanTransaction> accrualTransactions,LocalDate transactionDate,List<LoanTransaction> existingIncomeTransactionList,BigDecimal interestdue) {
        BigDecimal interest = BigDecimal.ZERO;
        BigDecimal fee = BigDecimal.ZERO;
        BigDecimal penalties = BigDecimal.ZERO;
        BigDecimal totalAmount = BigDecimal.ZERO;
        ...
        if (this.loanInterestRecalculationDetails.getInterestRecalculationCompoundingMethod().equals(
            ...
            interest = compoundingDetail.getAmount();
                ...
                InterestRecalculationCompoundingMethod.FEE)) {
            determineFeeDetails(lastCompoundingDate, transactionDate, feeDetails);
            fee = (BigDecimal) feeDetails.get("fee");
            penalties = (BigDecimal) feeDetails.get("penalties");
        } else if (this.loanInterestRecalculationDetails.getInterestRecalculationCompoundingMethod().equals(
                InterestRecalculationCompoundingMethod.INTEREST_AND_FEE)) {
            ...
            LocalDate newDate=lastCompoundingDate.plusDays(1);
            if(newDate.getMonthOfYear()==transactionDate.getMonthOfYear() && (newDate.isBefore(transactionDate)|| (newDate.equals(transactionDate))))
            {
            ...
            totalAmount = totalAmount.add(fee).add(penalties).add(interest);
        ...
        if (existingIncomeTransaction == null) {
           ...
           LoanTransaction interestTransaction = LoanTransaction.incomePosting(this, this.getOffice(), transactionDate
                   .toDate(), interest, interest, fees, penality, currentUser);
           addLoanTransaction(interestTransaction);
       	}
       	if(!fee.equals(BigDecimal.ZERO)) {
       		 BigDecimal interests= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;       		
                LoanTransaction feeTransaction = LoanTransaction.incomePosting(this, this.getOffice(), transactionDate
                        .toDate(), fee, interests, fee, penality, currentUser);
                addLoanTransaction(feeTransaction);
                updateLoanChargesPaidBy(feeTransaction, feeDetails, null);
       	}
       	if(!penalties.equals(BigDecimal.ZERO)) {
      		 BigDecimal interests= BigDecimal.ZERO;BigDecimal fees= BigDecimal.ZERO;
               LoanTransaction penaltiesTransaction = LoanTransaction.incomePosting(this, this.getOffice(), transactionDate
                       .toDate(), penalties, interests, fees, penalties, currentUser);
               addLoanTransaction(penaltiesTransaction);
               updateLoanChargesPaidBy(penaltiesTransaction, feeDetails, null);
      	}
           BigDecimal fees= BigDecimal.ZERO;
           BigDecimal penality= BigDecimal.ZERO;
          LoanTransaction accrual = LoanTransaction.accrueInterestTransaction(this, this.getOffice(), transactionDate,
        		  interest, interest, fees, penality, currentUser);
           addLoanTransaction(accrual);*/
        	if(!interest.equals(BigDecimal.ZERO) && interest.compareTo(compInterest) != 0) {
         		 BigDecimal fees= BigDecimal.ZERO;BigDecimal penality= BigDecimal.ZERO;
         		 for(LoanTransaction transaction : exInterestIncomeTransaction) {   
    ...
    private List<LoanTransaction> getTransactionListForDate(List<LoanTransaction> transactions, LocalDate effectiveDate) {
        List<LoanTransaction> transaction = new ArrayList<>();;
    	for (LoanTransaction loanTransaction : transactions) {
            if (loanTransaction.getTransactionDate().isEqual(effectiveDate)) { transaction.add(loanTransaction); }
        }
        return transaction;
    }
    ...
    private void updateLoanOutstandingBalaces() {
        ...
        for (LoanTransaction loanTransaction : loanTransactions) {
            if (loanTransaction.isDisbursement() || loanTransaction.isIncomePosting()
            		|| loanTransaction.isManualCharges()) {
                ...
                if (this.loanInterestRecalculationDetails != null
                	...
                	if(loanTransaction.isRecoveryRepayment()) {
                		if(outstanding.isGreaterThan(loanTransaction.getAmount(getCurrency()))) {
                		outstanding = outstanding.minus(loanTransaction.getAmount(getCurrency()));
                		}
                		else if(outstanding.isEqualTo(Money.zero(getCurrency()))) {
                			outstanding = Money.zero(getCurrency());
                		}
                		else {
                			outstanding = outstanding.minus(loanTransaction.getAmount(getCurrency()));
                		}
    ...
    public BigDecimal updateLoanOutstandingBalacesForManualCharges(BigDecimal amount) {
        Money outstanding = Money.zero(getCurrency());
        List<LoanTransaction> loanTransactions = retreiveListOfTransactionsExcludeAccruals();
        for (LoanTransaction loanTransaction : loanTransactions) {
        	 if (loanTransaction.isDisbursement() || loanTransaction.isIncomePosting() 
    ...
    /**Habile get Total Interest Posting**/
    public Money getReceivableInterestIncomePostingForInstallment(final LocalDate tillDate) {
        Money receivableInterest = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (!transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate) && transaction.getTransactionDate().getMonthOfYear() == tillDate.getMonthOfYear()) {
                if (transaction.isIncomePosting()) {
                    receivableInterest = receivableInterest.plus(transaction.getInterestPortion(getCurrency()));
                } 
            }
    ...
    /**Habile get Total Fee Posting**/
    public Money getReceivableFeeIncomePosting(final LocalDate tillDate) {
        Money receivableFee = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (transaction.isNotReversed() && !transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate)) {
                if (transaction.isIncomePosting()) {
                	receivableFee = receivableFee.plus(transaction.getFeeChargesPortion(getCurrency()));
                } else if (transaction.isRepayment() || transaction.isInterestWaiver()) {
                	receivableFee = receivableFee.minus(transaction.getFeeChargesPortion(getCurrency()));
                }
            }
            if (receivableFee.isLessThanZero()) {
            	receivableFee = receivableFee.zero();
    ...
    /**Habile get Total Interest Posting**/
    public Money getReceivableFeeIncomePostingForInstallment(final LocalDate tillDate) {
        Money receivableFee = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (!transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate) && transaction.getTransactionDate().getMonthOfYear() == tillDate.getMonthOfYear()) {
    ...
    /**Habile get Total Penality Posting**/
    public Money getReceivablePenaltyIncomePosting(final LocalDate tillDate) {
        Money receivablePenalty = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (transaction.isNotReversed() && !transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate)) {
                if (transaction.isIncomePosting()) {
                	receivablePenalty = receivablePenalty.plus(transaction.getPenaltyChargesPortion(getCurrency()));
                } else if (transaction.isRepayment() || transaction.isInterestWaiver()) {
                	receivablePenalty = receivablePenalty.minus(transaction.getPenaltyChargesPortion(getCurrency()));
                }
            }
            if (receivablePenalty.isLessThanZero()) {
            	receivablePenalty = receivablePenalty.zero();
    ...
    /**Habile get Total Penality Posting**/
    public Money getReceivablePenaltyIncomePostingForInstallment(final LocalDate tillDate) {
        Money receivablePenalty = Money.zero(getCurrency());
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (!transaction.isRepaymentAtDisbursement() && !transaction.isDisbursement()
                    && !transaction.getTransactionDate().isAfter(tillDate) && transaction.getTransactionDate().getMonthOfYear() == tillDate.getMonthOfYear()) {
    ...
    public LoanSummaryWrapper getLoanSummaryWrapper() {
        return this.loanSummaryWrapper;
    }
    
    public LoanSummaryWrapper setLoanSummaryWrapper(LoanSummaryWrapper loanSummaryWrapper) {
        return this.loanSummaryWrapper=loanSummaryWrapper ;
    }
    ...
    public void setLoanSubStatus(final Integer loanSubStatus) {
