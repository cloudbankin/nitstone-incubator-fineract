...
public class DecliningBalanceInterestLoanScheduleGenerator extends AbstractLoanScheduleGenerator {
    ...
    public PrincipalInterest calculatePrincipalInterestComponentsForPeriod(final PaymentPeriodsInOneYearCalculator calculator,
            ...
            final LocalDate periodStartDate, final LocalDate periodEndDate, final Collection<LoanTermVariationsData> termVariations, final Loan loan) {
            ...
            for (Map.Entry<LocalDate, Money> principal : principalVariation.entrySet()) {
	                ...
	                if (!principal.getKey().isAfter(periodEndDate)) {
	                    ...
	                    if (interestForDays > 0) {
	                    	...
	                    	{
	                    		if(loan.getLoanTransactions().isEmpty())
	                    		{
	                    			final PrincipalInterest result = loanApplicationTerms.calculateTotalInterestForPeriod(calculator,
			                                interestCalculationGraceOnRepaymentPeriodFraction, periodNumber, mc, cumulatingInterestDueToGrace,
			                                balanceForInterestCalculation, interestStartDate, principal.getKey());
	                    		...
	                    		else
	                    		{
	                    			...
	                    			List<LoanTransaction> monthLastTransactions = new ArrayList<LoanTransaction>();
	                    			
	                    			for(LoanTransaction loanTransaction : loan.getLoanTransactions())
	                    			{
	                    				if(periodStartDate.minusDays(1).equals(loanTransaction.getTransactionDate()) && 
	                    				loanTransaction.getTypeOf().equals(LoanTransactionType.INCOME_POSTING) && loanTransaction.isNotReversed() /*&&
	                    				loanTransaction.getInterestPortion(loan.getCurrency()).isGreaterThan(Money.zero(loan.getCurrency()))*/)
	                    				{
	                    					
	                    					monthLastTransactions.add(loanTransaction);
	                    				}	
	                    				
	                    			}
	                    			
	                    			if(!monthLastTransactions.isEmpty())
	                    			{
	                    					int size = monthLastTransactions.size();
	                    					BigDecimal oustandingBal = monthLastTransactions.get(size-1).getOutstandingLoanBalance();
	                    					int totalTransactions = loan.getLoanTransactions().size();
	                    					LoanTransaction latestTransaction = loan.getLoanTransactions().get(totalTransactions-1);
	                    					if(latestTransaction.getTypeOf() == LoanTransactionType.REPAYMENT && latestTransaction.isNotReversed())
	                    					{	
	                    						if(latestTransaction.getTransactionDate().getMonthOfYear() == monthLastTransactions.get(size-1).getTransactionDate().getMonthOfYear()
	                    						 && latestTransaction.getTransactionDate().getYear() == monthLastTransactions.get(size-1).getTransactionDate().getYear())
	                    								oustandingBal = oustandingBal.subtract(latestTransaction.getAmount(loan.getCurrency()).getAmount());
	                    					}
	                    					
	                    					final PrincipalInterest result = loanApplicationTerms.calculateTotalInterestForPeriod(calculator,
	    			                                interestCalculationGraceOnRepaymentPeriodFraction, periodNumber, mc, cumulatingInterestDueToGrace,
	    			                                Money.of(loan.getCurrency(), oustandingBal), interestStartDate, principal.getKey());
	    			                        interestForThisInstallment = interestForThisInstallment.plus(result.interest());
	    			                        cumulatingInterestDueToGrace = result.interestPaymentDueToGrace();
	    			                        interestStartDate = principal.getKey();
        ...
        else
    	{
    		...
    		else
    		{
    			...
    			if(!monthLastTransactions.isEmpty())
    			{
    					...
    					final PrincipalInterest result = loanApplicationTerms.calculateTotalInterestForPeriod(calculator,
                                interestCalculationGraceOnRepaymentPeriodFraction, periodNumber, mc, cumulatingInterestDueToGrace,
                                Money.of(loan.getCurrency(), oustandingBal), interestStartDate, periodEndDate);
                        interestForThisInstallment = interestForThisInstallment.plus(result.interest());
                        cumulatingInterestDueToGrace = result.interestPaymentDueToGrace();
                        count1 ++;
