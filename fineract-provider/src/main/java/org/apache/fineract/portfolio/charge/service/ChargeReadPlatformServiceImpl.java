...
public class ChargeReadPlatformServiceImpl implements ChargeReadPlatformService {
    ...
    public ChargeReadPlatformServiceImpl(final CurrencyReadPlatformService currencyReadPlatformService,
            ...
            final AccountingDropdownReadPlatformService accountingDropdownReadPlatformService,/*final ChargeSlabDetailsRepository chargeSlabDetailsRepository,*/
            final TaxReadPlatformService taxReadPlatformService,final ChargeAdditionalDetailsRepository chargeAdditionalDetailsRepository) {
...
* 
* This method is used to get the slab values
* 
*/
/*@Override
public List<ChargeSlabDetails> retriveSlabDetails(final Long chargeId){
	List<ChargeSlabDetails> listOfSlabs = this.chargeSlabDetailsRepository.findByChargeId(chargeId);
	return listOfSlabs;
}*/
@Override
public ChargeAdditionalDetails getChargeAdditionalDetails(final Long chargeId){
	ChargeAdditionalDetails chargeAdditionalDetails=null;
	try{
		chargeAdditionalDetails=this.chargeAdditionalDetailsRepository.retriveAdditionalDetails(chargeId);
	}catch (Exception e) {
        throw new ChargeNotFoundException(chargeId);
    }
 ...
 */
    ...
    /*
     * Habile changes
     * Here we retrieving only the extension charges
     */
    @Override
    public Collection<ChargeData> retrieveLoanApplicableExtensionFeeCharges() {
        final ChargeMapper rm = new ChargeMapper();

        String sql = "select " + rm.chargeSchema()
                + " where c.is_deleted=false and c.is_active=true and c.is_penalty=false and c.charge_applies_to_enum=? ";
        sql += " and c.charge_time_enum = 17 ";
        sql += addInClauseToSQL_toLimitChargesMappedToOffice_ifOfficeSpecificProductsEnabled();
        sql += " order by c.name ";
        return this.jdbcTemplate.query(sql, rm, new Object[] { ChargeAppliesTo.LOAN.getValue() });
    }
