...
public class AddressReadPlatformServiceImpl implements AddressReadPlatformService {
	...
	private static final class AddMapper implements RowMapper<AddressData> {
		public String schema() {
			return "addr.address_type as addressType,ca.client_id as client_id,addr.id as id,ca.address_type_id as addresstyp,ca.is_active as is_active,addr.street as street,addr.address_line_1 as address_line_1,addr.address_line_2 as address_line_2,"
					+ "addr.address_line_3 as address_line_3,addr.town_village as town_village, addr.city as city,addr.county_district as county_district,"
	...
	public Collection<AddressData> retrieveAllClientAddress(final long clientid) {
		...
		final String sql = "select " + rm.schema() + " and ca.client_id=? where addr.is_active=1";
		return this.jdbcTemplate.query(sql, rm, new Object[] { clientid });
	}

	@Override
	public Collection<AddressData> retrieveByEntityId(final long entityId, String entityType) {
		this.context.authenticatedUser();
		final AddressMapper rm = new AddressMapper();
		final String sql = "select " + rm.schema();
		return this.jdbcTemplate.query(sql, rm, new Object[] { entityId, entityType });
	}

	// Habile changes retrieve address by address type
	@Override
	public AddressData retrieveByEntityIdAndAddressType(final long entityId, String entityType,
		...
		sql = sql.replace("group by id", " and addr.address_type = ? group by id limit 1");
		return this.jdbcTemplate.queryForObject(sql, rm, new Object[] { entityId, entityType, addressType });
	}
	// Habile changes end
	...
	@Override
	public Collection<AddressData> retrieveAddressbyId(long typeid) {
		this.context.authenticatedUser();

		final AddMapper rm = new AddMapper();
		// final String sql = "select " + rm.schema() + " where addr.id=?";

		// final String sql="select cv2.code_value as addressType,ca.client_id as
		...
		// address_line_1,addr.address_line_2 as address_line_2,addr.address_line_3 as
		// address_line_3,addr.town_village as town_village, addr.city as
		// city,addr.county_district as county_district,addr.state_province_id as
		...
		// postal_code,addr.latitude as latitude,addr.longitude as
		// longitude,addr.created_by as created_by,addr.created_on as
		// created_on,addr.updated_by as updated_by,addr.updated_on as updated_on from
		// m_address addr left join m_code_value cv on addr.state_province_id=cv.id left
		// join m_code_value c on addr.country_id=c.id join m_client_address ca join
		// m_code_value cv2 on ca.address_type_id=cv2.id and addr.id=? group by id";
		final String sql = "SELECT cv2.code_value AS addresstype, 0 AS client_id, addr.id AS id, cv2.id AS addresstyp, addr.is_active AS is_active, addr.street AS street, addr.address_line_1 AS address_line_1, addr.address_line_2 AS address_line_2, addr.address_line_3 AS address_line_3, addr.town_village AS town_village, addr.city AS city, addr.county_district AS county_district, addr.state_province_id AS state_province_id, cv.code_value AS state_name, addr.country_id AS country_id, c.code_value AS country_name, addr.postal_code AS postal_code, addr.latitude AS latitude, addr.longitude AS longitude, addr.created_by AS created_by, addr.created_on AS created_on, addr.updated_by AS updated_by, addr.updated_on AS updated_on FROM m_address addr LEFT JOIN m_code_value cv ON addr.state_province_id=cv.id LEFT JOIN m_code_value c ON addr.country_id=c.id JOIN m_code_value cv2 ON addr.address_type=cv2.code_value AND addr.id=?";
		return this.jdbcTemplate.query(sql, rm, new Object[] { typeid });
	}
	...
	private static final class AddressMapper implements RowMapper<AddressData> {
		...
		public AddressData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			final long client_id = 0l;// rs.getLong("client_id");

			final String street = rs.getString("street");

			final long address_type_id = 0l;// rs.getLong("addresstyp");
		...
		public String schema() {
			return " addr.address_type as addressType,addr.id as id,addr.street as street,addr.is_active as is_active,addr.address_line_1 as address_line_1,"
					+ "addr.address_line_2 as address_line_2,addr.address_line_3 as address_line_3,addr.town_village as town_village, "
					+ "addr.city as city,addr.county_district as county_district,addr.state_province_id as state_province_id,"
					+ "cv.code_value as state_name, addr.country_id as country_id,c.code_value as country_name,addr.postal_code as postal_code,addr.latitude as latitude,"
					+ "addr.longitude as longitude,addr.created_by as created_by,addr.created_on as created_on,"
					+ "addr.updated_by as updated_by,addr.updated_on as updated_on from m_address addr left join m_code_value cv on addr.state_province_id=cv.id left join  m_code_value c"
					+ " on addr.country_id=c.id join m_code_value cv2 where  addr.parent_entity_id=? and addr.parent_entity_type=? and addr.is_active=1 group by id ";
		}
	}
}
