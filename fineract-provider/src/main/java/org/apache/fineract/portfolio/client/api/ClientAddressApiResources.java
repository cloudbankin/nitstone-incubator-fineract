...
public class ClientAddressApiResources {
	...
	public String getAddresses(@QueryParam("status") final String status, @QueryParam("type") final long addressTypeId,
			...
			@QueryParam("entityId") final long entityId, @QueryParam("entityType") final String entityType) {
		...
		} else if (status.equalsIgnoreCase("300") && entityType != null) {
			address = this.readPlatformService.retrieveByEntityId(entityId, entityType);
		} else if (status.equalsIgnoreCase("200")) {
			address = this.readPlatformService.retrieveAddressbyId(addressTypeId);
		} else {
