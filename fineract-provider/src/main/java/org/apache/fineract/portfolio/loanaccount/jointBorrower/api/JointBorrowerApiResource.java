 ...
 * under the License.
 */
package org.apache.fineract.portfolio.loanaccount.jointBorrower.api;
...
@Component
@Scope("singleton")
public class JointBorrowerApiResource {

    private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList("id", "loanId",
            "clientRelationshipType",
            "borrowerType", "firstname", "lastname", "entityId", "externalId", "officeName", "joinedDate", "addressLine1", "addressLine2",
            "city", "state", "zip", "country", "mobileNumber", "housePhoneNumber", "comment", "dob", "borrowerTypeOptions",
    ...
    private final String resourceNameForPermission = "JOINTBORROWER";

    private final JointBorrowerReadPlatformService guarantorReadPlatformService;
    private final CodeValueReadPlatformService codeValueReadPlatformService;
    private final DefaultToApiJsonSerializer<JointBorrowerData> apiJsonSerializerService;
    ...
    @Autowired
    public JointBorrowerApiResource(final PlatformSecurityContext context, final JointBorrowerReadPlatformService guarantorReadPlatformService,
            final DefaultToApiJsonSerializer<JointBorrowerData> toApiJsonSerializer, final ApiRequestParameterHelper apiRequestParameterHelper,
    ...
    public String newGuarantorTemplate(@Context final UriInfo uriInfo) {
        ...
        final JointBorrowerData guarantorData = JointBorrowerData.template(guarantorTypeOptions, allowedClientRelationshipTypes,
    ...
    public String retrieveGuarantorDetails(@Context final UriInfo uriInfo, @PathParam("loanId") final Long loanId) {
        ...
        final List<JointBorrowerData> guarantorDatas = this.guarantorReadPlatformService.retrieveGuarantorsForValidLoan(loanId);
    ...
    public String retrieveGuarantorDetails(@Context final UriInfo uriInfo, @PathParam("loanId") final Long loanId,
        ...
        JointBorrowerData guarantorData = this.guarantorReadPlatformService.retrieveGuarantor(loanId, guarantorId);
        ...
        if (settings.isTemplate()) {
            ...
            final List<EnumOptionData> guarantorTypeOptions = GuarantorEnumerations.guarantorType(GuarantorType.values());
            final Collection<PortfolioAccountData> accountLinkingOptions = null;
            guarantorData = JointBorrowerData.templateOnTop(guarantorData, guarantorTypeOptions, allowedClientRelationshipTypes,
                    accountLinkingOptions);
        }
    ...
    public String createGuarantor(@PathParam("loanId") final Long loanId, final String apiRequestBodyAsJson) {
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createJointBorrower(loanId).withJson(apiRequestBodyAsJson).build();
    ...
    public String deleteGuarantor(@PathParam("loanId") final Long loanId, @PathParam("guarantorId") final Long guarantorId,
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().deleteJointBorrower(loanId, guarantorId, guarantorFundingId).build();
    ...
    public String accountsTemplate(@QueryParam("clientId") final Long clientId, @PathParam("loanId") final Long loanId,
        ...
        final JointBorrowerData guarantorData = JointBorrowerData.template(null, null, accountLinkingOptions);
        final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
