...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator {
    ...
    public LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
            final Set<LoanCharge> loanCharges, final HolidayDetailDTO holidayDetailDTO, final String istrue, final LoanProduct loanProduct) {
        final LoanScheduleParams loanScheduleRecalculationDTO = null;
        return generate(mc, loanApplicationTerms, loanCharges, holidayDetailDTO, loanScheduleRecalculationDTO, null, istrue, loanProduct);
    }

    // Habile Arivu new config
    private LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
            final Set<LoanCharge> loanCharges, final HolidayDetailDTO holidayDetailDTO, final LoanScheduleParams loanScheduleParams, final Loan loan,final String istrue,  final LoanProduct loanProduct) {
        ...
        /** Habile Changes **/
		LocalDate loanEndDateTemp = loanEndDate;
		int noOfDaysExcludeInterest = 0;
		for (LoanCharge lc : loanCharges) {
			if (lc.isPenaltyCharge() && !lc.isWaived() && lc.isOverdueInstallmentCharge()
					&& !(lc.getChargeCalculation().getValue() == 6)) {
				noOfDaysExcludeInterest = noOfDaysExcludeInterest + 1;
				loanEndDate = lc.getDueLocalDate();
			}
		}
		/** Habile Changes **/
		...
		LoanTermVariationsData lastDueDateVariation = loanApplicationTerms.getLoanTermVariations()
				.fetchLoanTermDueDateVariationsDataPayDayLoan();
        /*if (lastDueDateVariation != null) {
            loanEndDate = lastDueDateVariation.getDateValue();
        }*/
		/** Habile Changes **/
		// Get the date value from last due date variation and set the date as loan end
		// date
		if (lastDueDateVariation != null) {

			loanEndDateTemp = lastDueDateVariation.getDateValue();
			// loanEndDate = lastDueDateVariation.getDateValue();
		}

		if (!loanEndDate.isAfter(loanEndDateTemp)) {
			loanEndDate = loanEndDateTemp;
		}
		/** Habile Changes **/
        ...
        if(!loanProduct.isNewProductConfig())
        	firstRepaymentdate = this.scheduledDateGenerator.generateNextRepaymentDate(
        	...
        	firstRepaymentdate = this.scheduledDateGenerator.generateNextRepaymentDate(
                loanApplicationTerms.getExpectedDisbursementDate(), loanApplicationTerms, isFirstRepayment,'\0','\0');
        ...
        if (!scheduleParams.isPartialUpdate()) {
            ...
            if(istrue!=null)
            {
            	if(istrue.equals("true") && loanProduct.isNewProductConfig())
            	{
            		updateAmortization1(mc, loanApplicationTerms, scheduleParams.getPeriodNumber(), scheduleParams.getOutstandingBalance(),istrue);
            		
            		 newEmi=loanApplicationTerms.getFixedEmiAmount();
            	}	
        ...
        while (!scheduleParams.getOutstandingBalance().isZero() || !scheduleParams.getDisburseDetailMap().isEmpty()) {
            ...
            scheduleParams.setActualRepaymentDate(this.scheduledDateGenerator.generateNextRepaymentDate(
            	...
            	scheduleParams.setActualRepaymentDate(this.scheduledDateGenerator.generateNextRepaymentDate(
                        scheduleParams.getActualRepaymentDate(), loanApplicationTerms, isFirstRepayment,'\0',1)); }
            ...
            LocalDate periodStartDateApplicableForInterest = calculateInterestStartDateForPeriod(loanApplicationTerms,
                    ...
                    loanApplicationTerms.getExpectedDisbursementDate(), loanProduct);
            ...
            //Loan loan = loanCharges.iterator().next().getLoan();
            ...
            if (loanApplicationTerms.isInterestRecalculationEnabled()) {
                populateCompoundingDatesInPeriod(scheduleParams.getPeriodStartDate(), scheduledDueDate, loanApplicationTerms,
                        holidayDetailDTO, scheduleParams, loanCharges, currency, loanProduct);
            }

            // populates the collection with transactions till the due date of
            // the period for interest recalculation enabled loans
            Collection<RecalculationDetail> applicableTransactions = null;
            
            if(!loanProduct.isNewProductConfig())
            	applicableTransactions = getApplicableTransactionsForPeriod(
                    scheduleParams.applyInterestRecalculation(), scheduleParams.getCompondingDate(), transactions);
            ...
            handleRecalculationForNonDueDateTransactions(mc, loanApplicationTerms, loanCharges, holidayDetailDTO, scheduleParams, periods,
                    ...
                    periodStartDateApplicableForInterest, applicableTransactions, currentPeriodParams, loanProduct);
         ...
         // 5 determine principal,interest of repayment period
            PrincipalInterest principalInterestForThisPeriod = null;
                                
            LocalDate interestAtmonthEnd= LocalDate.now();
            LocalDate ActualperiodStartDateApplicableForInterest = LocalDate.now();
            ...
            else if(!loanProduct.isNewProductConfig())
            {
	            if(scheduleParams.getPeriodNumber()==1) { 
	            	 ...
	            	 interestAtmonthEnd = periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1);
	            	 ActualperiodStartDateApplicableForInterest= periodStartDateApplicableForInterest;
	            	 principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
	                         this.paymentPeriodsInOneYearCalculator, currentPeriodParams.getInterestCalculationGraceOnRepaymentPeriodFraction(),
	                         scheduleParams.getTotalCumulativePrincipal().minus(scheduleParams.getReducePrincipal()),
	                         scheduleParams.getTotalCumulativeInterest(), loanApplicationTerms.getTotalInterestDue(),
	                         scheduleParams.getTotalOutstandingInterestPaymentDueToGrace(), scheduleParams.getOutstandingBalanceAsPerRest(),
	                         loanApplicationTerms, scheduleParams.getPeriodNumber(), mc, mergeVariationsToMap(scheduleParams),
	                         scheduleParams.getCompoundingMap(), periodStartDateApplicableForInterest, interestAtmonthEnd, interestRates, loan);
	
	            }
	            else if(scheduleParams.getPeriodNumber()==loanApplicationTerms.getActualNoOfRepaymnets()) { // LAst installment Calculation           	 
	            	  ActualperiodStartDateApplicableForInterest = periodStartDateApplicableForInterest.withDayOfMonth(1);
	            	 principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
	                         this.paymentPeriodsInOneYearCalculator, currentPeriodParams.getInterestCalculationGraceOnRepaymentPeriodFraction(),
	                         scheduleParams.getTotalCumulativePrincipal().minus(scheduleParams.getReducePrincipal()),
	                         scheduleParams.getTotalCumulativeInterest(), loanApplicationTerms.getTotalInterestDue(),
	                         scheduleParams.getTotalOutstandingInterestPaymentDueToGrace(), scheduleParams.getOutstandingBalanceAsPerRest(),
	                         loanApplicationTerms, scheduleParams.getPeriodNumber(), mc, mergeVariationsToMap(scheduleParams),
	                         scheduleParams.getCompoundingMap(), ActualperiodStartDateApplicableForInterest, scheduledDueDate, interestRates, loan);
	
	            interestAtmonthEnd = scheduledDueDate;
	            ActualperiodStartDateApplicableForInterest = ActualperiodStartDateApplicableForInterest.minusDays(1);
	            }
	            else {
	            	interestAtmonthEnd = periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1).minusDays(1).plusDays(1);
	            	  ActualperiodStartDateApplicableForInterest = periodStartDateApplicableForInterest.withDayOfMonth(1);
	            	 principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
	                         this.paymentPeriodsInOneYearCalculator, currentPeriodParams.getInterestCalculationGraceOnRepaymentPeriodFraction(),
	                         scheduleParams.getTotalCumulativePrincipal().minus(scheduleParams.getReducePrincipal()),
	                         ...
	                         scheduleParams.getTotalOutstandingInterestPaymentDueToGrace(), scheduleParams.getOutstandingBalanceAsPerRest(),
	                         loanApplicationTerms, scheduleParams.getPeriodNumber(), mc, mergeVariationsToMap(scheduleParams),
	                         scheduleParams.getCompoundingMap(), ActualperiodStartDateApplicableForInterest, interestAtmonthEnd, interestRates, loan);
	            	 
	            	 interestAtmonthEnd = periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1).minusDays(1);
	                 ActualperiodStartDateApplicableForInterest = ActualperiodStartDateApplicableForInterest.minusDays(1);
	            }
            }
           ...
           /* principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
            ...
            //get IncomepOsting amount on month end
            ...
            /*if(loan !=null) {
            postingAmount = loan.getReceivableInterestIncomePosting(periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1).minusDays(1));
            feePostingAmount = loan.getReceivableFeeIncomePosting(periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1).minusDays(1));
            penaltyPostingAmount = loan.getReceivablePenaltyIncomePosting(periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1).minusDays(1));
            }*/
            ...
            // Habile Arivu last repayment correction starts
                    /*  if(scheduleParams.getPeriodNumber()==loanApplicationTerms.getActualNoOfRepaymnets() && !loanProduct.isNewProductConfig())
                       scheduleParams.setOutstandingBalance(Money.zero(currency));*/
            // Habile Arivu last repayment correction ends

            // applies charges for the period
            applyChargesForCurrentPeriodInstallment(loanCharges, currency, scheduleParams, ActualperiodStartDateApplicableForInterest, interestAtmonthEnd, currentPeriodParams);
            ...
            // Habile Arivu new config
            if(!loanProduct.isNewProductConfig())
            	addLoanRepaymentScheduleInstallment(scheduleParams.getInstallments(), installment ,postingAmount.getAmount(),feePostingAmount.getAmount(),penaltyPostingAmount.getAmount());
            ...
            installment = handleRecalculationForTransactions(mc, loanApplicationTerms, holidayDetailDTO, currency, scheduleParams,
                    ...
                    lastTotalOutstandingInterestPaymentDueToGrace, installment, loanCharges, loanProduct);
            ...
            updateAmountsWithEffectiveDate(loanApplicationTerms, holidayDetailDTO, scheduleParams, scheduledDueDate, currentPeriodParams,
                    installment, lastRestDate, loanProduct);
        ...
        if (scheduleParams.applyInterestRecalculation() && scheduleParams.getLatePaymentMap().size() > 0
            ...
            Money totalInterest = addInterestOnlyRepaymentScheduleForCurrentdate(mc, loanApplicationTerms, holidayDetailDTO, currency,
                    periods, currentDate, loanRepaymentScheduleTransactionProcessor, transactions, loanCharges, scheduleParams, loan, loanProduct);
        ...
        // Habile Arivu new config
        LoanScheduleModel loanScheduleModel = LoanScheduleModel.from(periods, applicationCurrency, scheduleParams.getLoanTermInDays(),
        ...
        loanScheduleModel.setNewEmi(newEmi);
    ...
    private void applyChargesForCurrentPeriod(final Set<LoanCharge> loanCharges, final MonetaryCurrency currency,
        ...
        currentPeriodParams.setFeeChargesForInstallment(cumulativeFeeChargesDueWithinstallment(scheduleParams.getPeriodStartDate(),
                scheduledDueDate, loanCharges, currency, principalInterest, scheduleParams.getPrincipalToBeScheduled(),
                scheduleParams.getTotalCumulativeInterest(), true,scheduleParams.getOutstandingBalance()));
    ...
    private void applyChargesForCurrentPeriodInstallment(final Set<LoanCharge> loanCharges, final MonetaryCurrency currency,
            LoanScheduleParams scheduleParams, LocalDate startDate, LocalDate scheduledDueDate, ScheduleCurrentPeriodParams currentPeriodParams) {
        PrincipalInterest principalInterest = new PrincipalInterest(currentPeriodParams.getPrincipalForThisPeriod(),
                currentPeriodParams.getInterestForThisPeriod(), null);
        currentPeriodParams.setFeeChargesForInstallment(cumulativeFeeChargesDueWithinstallment(startDate,
                scheduledDueDate, loanCharges, currency, principalInterest, scheduleParams.getPrincipalToBeScheduled(),
                scheduleParams.getTotalCumulativeInterest(), true,scheduleParams.getOutstandingBalance()));
        currentPeriodParams.setPenaltyChargesForInstallment(cumulativePenaltyChargesDueWithin(startDate,
                scheduledDueDate, loanCharges, currency, principalInterest, scheduleParams.getPrincipalToBeScheduled(),
                scheduleParams.getTotalCumulativeInterest(), true));
        scheduleParams.addTotalFeeChargesCharged(currentPeriodParams.getFeeChargesForInstallment());
        scheduleParams.addTotalPenaltyChargesCharged(currentPeriodParams.getPenaltyChargesForInstallment());
    ...
    private void updateAmountsWithEffectiveDate(final LoanApplicationTerms loanApplicationTerms, final HolidayDetailDTO holidayDetailDTO,
            LoanScheduleParams scheduleParams, LocalDate scheduledDueDate, ScheduleCurrentPeriodParams currentPeriodParams,
            LoanScheduleModelPeriod installment, LocalDate lastRestDate, final LoanProduct loanProduct) {
        ...
        updateCompoundingMap(loanApplicationTerms, holidayDetailDTO, scheduleParams, lastRestDate, scheduledDueDate, loanProduct);
    ...
    private LoanScheduleModelPeriod handleRecalculationForTransactions(final MathContext mc,
            ...
            final LoanScheduleModelPeriod installment, Set<LoanCharge> loanCharges, final LoanProduct loanProduct) {
                ...
                if (!detail.isProcessed()) {
                    ...
                    if (unprocessed.isGreaterThanZero()) {
                        ...
                        modifiedInstallment = handlePrepaymentOfLoan(mc, loanApplicationTerms, holidayDetailDTO, scheduleParams,
                                ...
                                lastTotalOutstandingInterestPaymentDueToGrace, transactionDate, modifiedInstallment, loanCharges, loanProduct);
    ...
    private LoanScheduleModelPeriod handlePrepaymentOfLoan(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
            ...
            final ScheduleCurrentPeriodParams currentPeriodParams, final Money lastTotalOutstandingInterestPaymentDueToGrace,
            final LocalDate transactionDate, final LoanScheduleModelPeriod installment, Set<LoanCharge> loanCharges, final LoanProduct loanProduct) {
    	LoanScheduleModelPeriod modifiedInstallment = installment;
        ...
        if (!oustanding.isGreaterThan(currentPeriodParams.getInterestForThisPeriod())
            ...
            populateCompoundingDatesInPeriod(periodStartDateApplicableForInterest, calculateTill, loanApplicationTerms, holidayDetailDTO,
                    scheduleParams, loanCharges, totalInterestChargedForFullLoanTerm.getCurrency(), loanProduct);
            ...
            PrincipalInterest interestTillDate = null;
            		if(!loanProduct.isNewProductConfig())
            		{
                        
            		interestTillDate = calculatePrincipalInterestComponentsForPeriod(this.paymentPeriodsInOneYearCalculator,
                    ...
                    periodStartDateApplicableForInterest, calculateTill, interestRates, loanCharges.iterator().next().getLoan());
            		}
            ...
            for (RecalculationDetail detail : applicableTransactions) {
                ...
                if (transactionDate.isBefore(scheduledDueDate)) {
                    if (scheduleParams.getLoanRepaymentScheduleTransactionProcessor() != null
                        ...
                        if (!transactionDate.isEqual(scheduleParams.getPeriodStartDate()) || scheduleParams.getInstalmentNumber() == 1) {
                            ...
                            if (daysInPeriodApplicable > 0) {
                                ...
                                principalInterestForThisPeriod=	calculatePrincipalInterestComponentsForPeriod(
                                        ...
                                        interestRates, loanCharges.iterator().next().getLoan());
                                }
                                else if(loanProduct.isNewProductConfig())
                                {	
                                principalInterestForThisPeriod=	calculatePrincipalInterestComponentsForPeriod(
                    	...
                    	if(!loanProduct.isNewProductConfig()) {
                         applicableDate = getNextRestScheduleDate(transactionDate.minusDays(1), loanApplicationTerms,
                                holidayDetailDTO);   
                        if(applicableDate.isEqual(transactionDate)) {
                        	applicableDate = getNextRestScheduleDate(transactionDate.plusDays(1), loanApplicationTerms,
                                    holidayDetailDTO);   
                        }
                        LocalDate compondingDate = periodStartDateApplicableForInterest.plusMonths(1).withDayOfMonth(1);
                     
	                        if (applicableDate.isBefore(scheduledDueDate)) {
	                        	 List<LoanTransaction> currentTransactions = createCurrentTransactionList(detail);
		                            Loan loan = currentTransactions.iterator().next().getLoan();
		                            Money unprocessed = Money.zero(currency);
		                            Money arrears = Money.zero(currency);
	                        	   if(applicableDate.isEqual(compondingDate)) {
			                            unprocessed = scheduleParams.getLoanRepaymentScheduleTransactionProcessor().handleRepaymentSchedule(
			                                    currentTransactions, currency, scheduleParams.getInstallments());
			                            
			                            arrears = fetchArrears(loanApplicationTerms, currency, detail.getTransaction());
	                        	   }
	                           /* if(scheduleParams.getInstalmentNumber() ==1) {
	                            	List<LoanRepaymentScheduleInstallment> installments = new ArrayList<LoanRepaymentScheduleInstallment>();
	                            	LoanRepaymentScheduleInstallment tempInstallment = loan.getRepaymentScheduleInstallments().get(0);
	                            	LocalDate compondingDate = tempInstallment.getFromDate().plusMonths(1).withDayOfMonth(1).minusDays(1);
	                            	if(!transactionDate.isBefore(compondingDate)){
		                            	tempInstallment.resetDerivedComponents();
		                            	installments.add(tempInstallment);
		                            	unprocessed = scheduleParams.getLoanRepaymentScheduleTransactionProcessor().handleRepaymentSchedule(
		                                         currentTransactions, currency, installments);
		                            	 LoanTransaction transaction = detail.getTransaction();
	                            		 transaction.resetInterestDerivedComponents();
		                            	 arrears = fetchArrears(loanApplicationTerms, currency, transaction);
	                            	}else {
	                            		unprocessed = scheduleParams.getLoanRepaymentScheduleTransactionProcessor().handleRepaymentSchedule(
	                                        currentTransactions, currency, scheduleParams.getInstallments());
	                            ...
	                            if (unprocessed.isGreaterThanZero()) {
	                                ...
	                                if (!scheduleParams.getOutstandingBalance().isGreaterThan(unprocessed)
	                                    ...
	                                    if (!principalInterestForThisPeriod.interest()
	                                        ...
	                                        LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null,
	                                                transactionDate, null, DateUtils.getLocalDateTimeOfTenant(), null, loan);
	                                ...
	                                LoanTransaction loanTransaction = LoanTransaction.repayment(loan, null, unprocessed, null, scheduledDueDate,
	                                ...
	                                if (unprocessed.isGreaterThanZero()) {
	                                    ...
	                                    if (!scheduleParams.getOutstandingBalance().isGreaterThan(unprocessed)
	                                        ...
	                                        if (!principalInterestForThisPeriod.interest()
	                                            ...
	                                            updateMapWithAmount(scheduleParams.getPrincipalPortionMap(), unprocessed.negated(), applicableDate);
	                                            LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null,
	                                                    transactionDate, null, DateUtils.getLocalDateTimeOfTenant(), null, loanCharges.iterator().next().getLoan());
	                                            RecalculationDetail recalculationDetail = new RecalculationDetail(transactionDate, loanTransaction);
	                                            unprocessedTransactions.add(recalculationDetail);
	                                            break;
	                                        }
	                                    ...
	                                    LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null, scheduledDueDate,
	                                            null, DateUtils.getLocalDateTimeOfTenant(), null, loanCharges.iterator().next().getLoan());
 ...
 // Habile Arivu new config
    private boolean updateFixedInstallmentAmount1(final MathContext mc, final LoanApplicationTerms loanApplicationTerms, int periodNumber,
            Money outstandingBalance,final String istrue) {
        boolean isAmountChanged = false;
        if (loanApplicationTerms.getActualFixedEmiAmount() == null && loanApplicationTerms.getInterestMethod().isDecliningBalnce()
                && loanApplicationTerms.getAmortizationMethod().isEqualInstallment()) {
            if (periodNumber < loanApplicationTerms.getPrincipalGrace() + 1) {
            ...
            Money emiAmount = loanApplicationTerms.pmtForInstallment1(this.paymentPeriodsInOneYearCalculator, outstandingBalance,
                    periodNumber, mc,istrue,loanApplicationTerms);
            loanApplicationTerms.setFixedEmiAmount(emiAmount.getAmount());
            isAmountChanged = true;
        }
        return isAmountChanged;
    ...
    private Money addInterestOnlyRepaymentScheduleForCurrentdate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
            ...
            final Loan loan, final LoanProduct loanProduct) {
        ...
        do {
        	...
        	if(!loanProduct.isNewProductConfig()) {
        		...
        		params.setActualRepaymentDate(this.scheduledDateGenerator.generateNextRepaymentDate(params.getActualRepaymentDate(),
                    loanApplicationTerms, isFirstRepayment,'\0','\0'));
        	}
            ...
            populateCompoundingDatesInPeriod(params.getPeriodStartDate(), params.getActualRepaymentDate(), loanApplicationTerms,
                    holidayDetailDTO, params, loanCharges, currency, loanProduct);
                ...
                if (!params.getPeriodStartDate().isEqual(transactionDate)) {
                	...
                	if(!loanProduct.isNewProductConfig())
                	{
                		principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                            ...
                            params.getPeriodStartDate(), transactionDate, applicableVariations, loan);
                	}
                    ...
                    updateCompoundingMap(loanApplicationTerms, holidayDetailDTO, params, lastRestDate, transactionDate, loanProduct);
            ...
            if (!outstanding.isZero()) {
                ...
                if(!loanProduct.isNewProductConfig()) {
                	principalInterestForThisPeriod= calculatePrincipalInterestComponentsForPeriod(
                        ...
                        params.getActualRepaymentDate(), applicableVariations, loan);
                }
    ...
    /*private void updateLatePaymentsToMap(final LoanApplicationTerms loanApplicationTerms, final HolidayDetailDTO holidayDetailDTO,
        ...
        for (LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment : installments) {
        	...
        	//LoanRepaymentScheduleInstallment installmentTemp = null;
        	
        	List <LoanTransaction> transactions = new ArrayList<LoanTransaction>();
          			
	        		if(loanRepaymentScheduleInstallment.getLoan()!= null)
	        			transactions = loanRepaymentScheduleInstallment.getLoan().getLoanTransactions();
        	...
        	{
	        	if(transactions.get(transactions.size()-1).getTypeOf() == LoanTransactionType.REPAYMENT)
	        		 transactionDate = transactions.get(transactions.size()-1).getTransactionDate();
        	}
        	
            LoanRepaymentScheduleInstallment changeInstallment = null;
            
            if(loanRepaymentScheduleInstallment.getLoan()!= null)
        	{
	            for (LoanRepaymentScheduleInstallment installment : loanRepaymentScheduleInstallment.getLoan().getRepaymentScheduleInstallments()) {
	            	if(transactionDate != null)
	            	{	
		            	if(transactionDate.getMonthOfYear()== installment.getDueDate().getMonthOfYear()
		            		&& transactionDate.getYear()== installment.getDueDate().getYear())
		            	{
		            		if(transactionDate.isBefore(installment.getDueDate())
		                			|| transactionDate.isEqual(installment.getDueDate()))
            ...
            if (loanRepaymentScheduleInstallment.isNotFullyPaidOff()
                ...
                if (principalEffectiveDate.isBefore(currentDate)) {
                	...
                	if(changeInstallment != null)
                	{
                		
			                	if(transactionDate.isBefore(changeInstallment.getDueDate())
			                			|| transactionDate.isEqual(changeInstallment.getDueDate()))
			                	{
			                		 updateMapWithAmount(latePaymentMap, loanRepaymentScheduleInstallment.getPrincipal(currency),
			                                 principalEffectiveDate);
			                         totalCompoundingAmount = totalCompoundingAmount
			                                 .plus(loanRepaymentScheduleInstallment.getPrincipal(currency));
			                	//}
    ...
    private void updateCompoundingMap(final LoanApplicationTerms loanApplicationTerms, final HolidayDetailDTO holidayDetailDTO,
            final LoanScheduleParams params, final LocalDate lastRestDate, final LocalDate scheduledDueDate, final LoanProduct loanProduct) {
        if (loanApplicationTerms.isInterestRecalculationEnabled()
                ...
                if (params.getCompoundingDateVariations().containsKey(loanRepaymentScheduleInstallment.getFromDate())) {
                    ...
                    while (!compoundingEffectiveDate.isAfter(loanRepaymentScheduleInstallment.getDueDate())) {
                        if (compoundingEffectiveDate.isEqual(loanRepaymentScheduleInstallment.getDueDate())) {
                            ...
                            if(!loanProduct.isNewProductConfig())
                            {	
	                            Money amountToBeCompounding = amountCharged.minus(compounedAmount);
	                            updateMapWithAmount(compoundingMap, amountToBeCompounding, compoundingEffectiveDate);
	                            totalCompoundedAmount = totalCompoundedAmount.plus(amountToBeCompounding.plus(compounedAmount));
                            }
                            else if(loanProduct.isNewProductConfig())
                            	totalCompoundedAmount = totalCompoundedAmount.plus(compounedAmount);
                        }

                        if (!loanApplicationTerms.allowCompoundingOnEod() && !loanProduct.isNewProductConfig()) {
    ...
    private void adjustCompoundedAmountWithPaidDetail(final LoanScheduleParams params, final LocalDate lastRestDate,
            ...
            HolidayDetailDTO holidayDetailDTO, final LoanProduct loanProduct) {
        for (LoanTransaction loanTransaction : transactions) {
             LocalDate amountApplicableDate = getNextRestScheduleDate(loanTransaction.getTransactionDate().minusDays(1),
                    loanApplicationTerms, holidayDetailDTO);
            
             // Habile Arivu new config
            if(amountApplicableDate.isEqual(loanTransaction.getTransactionDate()) && !loanProduct.isNewProductConfig()) {
            	            	    amountApplicableDate = getNextRestScheduleDate(loanTransaction.getTransactionDate().plusDays(1),
            	                      loanApplicationTerms, holidayDetailDTO);
            	            }
    ...
    private void populateCompoundingDatesInPeriod(final LocalDate startDate, final LocalDate endDate,
            ...
            final LoanScheduleParams scheduleParams, final Set<LoanCharge> charges, MonetaryCurrency currency, final LoanProduct loanProduct) {
        if (loanApplicationTerms.getInterestRecalculationCompoundingMethod().isCompoundingEnabled()) {
            ...
            while (compoundingDate.isBefore(endDate)) {
                ...
                if (compoundingDate.isBefore(endDate)) {
                    ...
                    Money penaltyChargesForInstallment = Money.zero(currency);
                    penaltyChargesForInstallment = cumulativePenaltyChargesDueWithin(lastCompoundingDate, compoundingDate, charges,
                ...
                if(!loanProduct.isNewProductConfig())
                	scheduleParams.setCompondingDate(compoundingDate);
            }
        }
    }
    ...
    private LocalDate calculateInterestStartDateForPeriod(final LoanApplicationTerms loanApplicationTerms, LocalDate periodStartDate,
            ...
            final Boolean isInterestChargedFromDateSameAsDisbursalDateEnabled, final LocalDate expectedDisbursementDate, final LoanProduct loanProduct) {
    ...
    public abstract PrincipalInterest calculatePrincipalInterestComponentsForPeriod(PaymentPeriodsInOneYearCalculator calculator,
            ...
            Collection<LoanTermVariationsData> termVariations, Loan loan);
    ...
    /** Habile changes to get loan details */
   ...
   private Money cumulativeFeeChargesDueWithin(final LocalDate periodStart, final LocalDate periodEnd, final Set<LoanCharge> loanCharges,
    ...
    private Money cumulativeFeeChargesDueWithinstallment(final LocalDate periodStart, final LocalDate periodEnd, final Set<LoanCharge> loanCharges,
            final MonetaryCurrency monetaryCurrency, final PrincipalInterest principalInterestForThisPeriod,
            final Money principalDisbursed, final Money totalInterestChargedForFullLoanTerm, boolean isInstallmentChargeApplicable,Money outStanding) {
            ...
            if (!loanCharge.isDueAtDisbursement() && loanCharge.isFeeCharge()) {
                if (loanCharge.isInstalmentFee() && isInstallmentChargeApplicable) {
                    cumulative = calculateInstallmentCharge(principalInterestForThisPeriod, cumulative, loanCharge);
                        ...
                        && loanCharge.getChargeCalculation().isPercentageBased()) {
                    cumulative = cumulative.plus(loanCharge.chargeAmount());
                } else if(loanCharge.isPrePayment()) {
    ...
    private Money calculateSpecificDueDateChargeWithPercentage(final Money principalDisbursed,
        ...
        /** Habile changes calculation extension service fee */
		LoanChargeAdditionalDetails lcad = loanCharge.getLoanChargeAdditionalDetails();

		if (loanCharge != null && LoanCharge.getChargeAdditionalDetailsBean() != null
				&& LoanCharge.getChargeAdditionalDetailsBean().getIsEnabledFeeCalculationBasedOnTenure() == 1) {
			loanChargeAmt = loanCharge.amount();
		} else if (loanCharge != null && lcad != null && !loanCharge.isInstallmentRescheduled()
				&& lcad.getIsEnabledFeeCalculationBasedOnTenure() == 1) {
			if (lcad.getNumberOfDays() != null) {
				if (loanCharge.getLoan().status().getValue() <= 200) {
					int days = Days.daysBetween(getLoanApplicationTerms().getExpectedDisbursementDate(),
							getLoanApplicationTerms().getLoanEndDate()).getDays();
					lcad.setNumberOfDays(days);
					loanChargeAmt = loanChargeAmt.multiply(BigDecimal.valueOf(lcad.getNumberOfDays()));
				} else {
					loanChargeAmt = loanChargeAmt.multiply(BigDecimal.valueOf(lcad.getNumberOfDays()));
				}
			} else {
			...
			if (loanCharge.getChargeCalculation().ispercentageOfOutstandingLoanAmount()
					&& loanCharge.isInstallmentRescheduled() && lcad.getIsEnabledFeeCalculationBasedOnTenure() == 1) {
				// loanChargeAmt = loanCharge.amount();
				loanChargeAmt = loanChargeAmt.multiply(new BigDecimal(lcad.getNumberOfDays()));
			}
		}
		/** Habile changes end */
		...
		if (loanCharge.getCharge().getTaxGroup() != null && lcad != null) {
			...
			if (lcad.getIsTaxIncluded() == 1) {
				while (taxGroupMappings.hasNext()) {
					...
					if (taxGroupMapping.getTaxComponent().getTaxComponentAdditionalDetails().getTaxComponentType()
							.equalsIgnoreCase(lcad.getTaxComponentType())) {
			...
			Iterator<TaxGroupMappings> taxMappings = loanCharge.getCharge().getTaxGroup().getTaxGroupMappings()
					.iterator();
			while (taxMappings.hasNext()) {
				...
				if (taxGroupMapping.getTaxComponent().getTaxComponentAdditionalDetails().getTaxComponentType()
					...
					if (lcad.getIsTaxIncluded() == 1) {
						...
						taxAmount = loanChargeAmt.multiply(taxComponentPercentage).divide(new BigDecimal(100))
								.setScale(0, RoundingMode.HALF_EVEN);
					}
					...
					FinabileLoanChargeTaxDetails finabileLoanChargeTaxDetails = new FinabileLoanChargeTaxDetails(
							loanCharge, taxGroupMapping.getTaxComponent(), taxGroupMapping.getTaxComponent().getName(),
							taxAmount);
...
//			lcad.setFeesComponent(feesComponent);
//			lcad.setTaxComponent(taxComponent);
    ...
    private LoanScheduleDTO rescheduleNextInstallments(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
            ...
            for (LoanRepaymentScheduleInstallment installment : processInstallmentsInstallments) {
                ...
                if (installment.getDueDate().isAfter(lastInstallmentDate)) {
                   ...
                   // check for date changes
                    ...
                    do {
                    	if(loan.getLoanProduct()!=null && !loan.getLoanProduct().isNewProductConfig()) {
                    		actualRepaymentDate = this.scheduledDateGenerator.generateNextRepaymentDate(actualRepaymentDate,
                                loanApplicationTerms, isFirstRepayment,'\0',1);
                    		}
                    	
                    	else if(loan.getLoanProduct()!=null && loan.getLoanProduct().isNewProductConfig()) {
                    		actualRepaymentDate = this.scheduledDateGenerator.generateNextRepaymentDate(actualRepaymentDate,
                                loanApplicationTerms, isFirstRepayment);}
                ...
                if (loanApplicationTerms.isInterestRecalculationEnabled()) {
                    ...
                    Money unprocessed = updateEarlyPaidAmountsToMap(loanApplicationTerms, holidayDetailDTO,
                            ...
                            installment, applicableTransactions, actualPrincipalPortion, loan.getLoanProduct());
                    ...
                    uncompoundedAmount = updateCompoundingDetailsForPartialScheduleGeneration(installment, loanApplicationTerms,
                            ...
                            holidayDetailDTO, loan.getLoanProduct());
                    ...
                    updateLatePaymentsToMap(loanApplicationTerms, holidayDetailDTO, currency, latePaymentMap, lastInstallmentDate,
                    	 ...
                    	 //newRepaymentScheduleInstallments.remove(installment);
                    	 //installment.resetDerivedComponents();
                    	 //newRepaymentScheduleInstallments.add(installment);
        ...
        // Habile Arivu new config
        LoanScheduleModel loanScheduleModel = generate(mc, loanApplicationTerms, loan.charges(), holidayDetailDTO, loanScheduleParams, loan, null, loan.getLoanProduct());
    ...
    private Money updateEarlyPaidAmountsToMap(final LoanApplicationTerms loanApplicationTerms, final HolidayDetailDTO holidayDetailDTO,
            ...
            Collection<RecalculationDetail> applicableTransactions, Money actualPrincipalPortion, final LoanProduct loanProduct) {
        Money unprocessed = Money.zero(currency);
        ...
        for (RecalculationDetail detail : applicableTransactions) {
            if (!detail.isProcessed()) {
                ...
                if (totalEarlyPayment.isGreaterThanZero()) {
            	  ...
            	  // Habile Arivu new config
            	  if(loanProduct !=null && !loanProduct.isNewProductConfig()) {
                	if(totalEarlyPayment.isLessThan(Money.zero(currency))) { 
                	  unprocessed = principalPaidWithTransaction;
                	}
                	else {
                		unprocessed = Money.zero(currency);
                	}
            	  }
                }
                // updates principal portion map with the early
                ...
                LocalDate applicableDate = getNextRestScheduleDate(detail.getTransactionDate().minusDays(1), loanApplicationTerms,
                        holidayDetailDTO);
                // Habile Arivu new config
                if(applicableDate.isEqual(detail.getTransactionDate()) && loanProduct !=null && !loanProduct.isNewProductConfig()) {
                	 applicableDate = getNextRestScheduleDate(detail.getTransactionDate().plusDays(1), loanApplicationTerms,
                            holidayDetailDTO);
                }
                updateMapWithAmount(principalPortionMap, unprocessed, applicableDate);
    ...
    private Money updateCompoundingDetailsForPartialScheduleGeneration(final LoanRepaymentScheduleInstallment installment,
        ...
        LocalDate applicableDate = LocalDate.now();
        for (RecalculationDetail detail : applicableTransactions) {
             applicableDate = getNextRestScheduleDate(detail.getTransactionDate().minusDays(1), loanApplicationTerms,
                    holidayDetailDTO);
            if(applicableDate.isEqual(detail.getTransactionDate()) && loanProduct !=null && !loanProduct.isNewProductConfig()) {
            	 applicableDate = getNextRestScheduleDate(detail.getTransactionDate().plusDays(1), loanApplicationTerms,
                        holidayDetailDTO);
    ...
    // Habile Arivu new config
    private void updateAmortization1(final MathContext mc, final LoanApplicationTerms loanApplicationTerms, int periodNumber,
            Money outstandingBalance,final String istrue) {
        if (loanApplicationTerms.getAmortizationMethod().isEqualInstallment()) {
            updateFixedInstallmentAmount1(mc, loanApplicationTerms, periodNumber, outstandingBalance,istrue);
        } else {
            loanApplicationTerms.updateFixedPrincipalAmount(mc, periodNumber, outstandingBalance);
        }
    }
    ...
    //Add interest poting amount with installment
    private LoanRepaymentScheduleInstallment addLoanRepaymentScheduleInstallment(final List<LoanRepaymentScheduleInstallment> installments,
            final LoanScheduleModelPeriod scheduledLoanInstallment ,BigDecimal postingAmount,BigDecimal feeamount,BigDecimal penaltyAmount) {
        LoanRepaymentScheduleInstallment installment = null;
        if (scheduledLoanInstallment.isRepaymentPeriod()) {
            installment = new LoanRepaymentScheduleInstallment(null, scheduledLoanInstallment.periodNumber(),
                    scheduledLoanInstallment.periodFromDate(), scheduledLoanInstallment.periodDueDate(),
                    scheduledLoanInstallment.principalDue(), scheduledLoanInstallment.interestDue(),
                    scheduledLoanInstallment.feeChargesDue(), scheduledLoanInstallment.penaltyChargesDue(),
                    scheduledLoanInstallment.isRecalculatedInterestComponent(), scheduledLoanInstallment.getLoanCompoundingDetails(),postingAmount,feeamount,penaltyAmount);
            installments.add(installment);
        }
        return installment;
    }
