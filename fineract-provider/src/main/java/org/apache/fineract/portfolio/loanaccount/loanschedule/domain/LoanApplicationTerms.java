...
public final class LoanApplicationTerms {
    ...
    public Money pmtForInstallment1(final PaymentPeriodsInOneYearCalculator calculator, final Money outstandingBalance,
            final int periodNumber, final MathContext mc,final String istrue,final LoanApplicationTerms loanApplicationTerms) {
        // Calculate exact period from disbursement date
//    	final LocalDate periodStartDate=loanApplicationTerms.expectedDisbursementDate;
//    	final LocalDate periodEndDate=loanApplicationTerms.seedDate;
        final LocalDate periodStartDate = getExpectedDisbursementDate().withDayOfMonth(1);
        final LocalDate periodEndDate = getPeriodEndDate(periodStartDate);
        // equal installments
        final int periodsElapsed = periodNumber - 1;
        ...
        final BigDecimal periodicInterestRateForRepaymentPeriod = periodicInterestRate(calculator, mc, DaysInMonthType.DAYS_30,
                DaysInYearType.DAYS_365, periodStartDate, periodEndDate, true);
        Money totalPmtForThisInstallment = calculateTotalDueForEqualInstallmentRepaymentPeriod1(periodicInterestRateForRepaymentPeriod,
                outstandingBalance, periodsElapsed,istrue);
        return totalPmtForThisInstallment;
    }
    ...
    /**Habile changes getting loan end date*/
...
//        numPeriods = numPeriods - this.periodsCompleted;
    ...
    // Habile Arivu new config
    private double paymentPerPeriod1(final BigDecimal periodicInterestRate, final Money balance, final int periodsElapsed,final String istrue) {
        ...
        if (getFixedEmiAmount() == null) {
            ...
            double interestcal=periodicInterestRate.doubleValue();
            
//            if(istrue!=null)
//            {
//            	//String istrue1=this.fromApiJsonHelper.extractStringNamed("isFixedAmount", istrue);
//            	if(istrue.equals("true"))
//            	{
//                 interestcal = (this.annualNominalInterestRate.divide(new BigDecimal(100))).multiply(new BigDecimal(0.08463638094)).doubleValue();
//            	}
//            	
//            	//this.fromApiJsonHelper.extractBooleanNamed("isFixedAmount", element);
//            }

            double installmentAmount = FinanicalFunctions.pmt(interestcal, periodsRemaining.doubleValue(),
                    principalDouble, futureValue, false);
    ...
    public Money calculateDecliningInterestDueForInstallmentBeforeApplyingGracePeriod(final PaymentPeriodsInOneYearCalculator calculator,
    ...
    // Habile Arivu new config
    private Money calculateTotalDueForEqualInstallmentRepaymentPeriod1(final BigDecimal periodicInterestRate, final Money balance,
            final int periodsElapsed,final String istrue) {

        final double paymentPerRepaymentPeriod = paymentPerPeriod1(periodicInterestRate, balance, periodsElapsed,istrue);

        return Money.of1(balance.getCurrency(), BigDecimal.valueOf(paymentPerRepaymentPeriod),istrue);
    }
