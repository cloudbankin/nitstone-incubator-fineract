...
@Service
public class JointBorrowerReadPlatformServiceImpl implements JointBorrowerReadPlatformService {
    ...
    @Autowired
    public JointBorrowerReadPlatformServiceImpl(final RoutingDataSource dataSource, final ClientReadPlatformService clientReadPlatformService,
    ...
    @Override
    public List<JointBorrowerData> retrieveGuarantorsForValidLoan(final Long loanId) {
        this.loanRepositoryWrapper.findOneWithNotFoundDetection(loanId);
        return retrieveGuarantorsForLoan(loanId);
    }

    @Override
    public List<JointBorrowerData> retrieveGuarantorsForLoan(final Long loanId) {
        final GuarantorMapper rm = new GuarantorMapper();
        String sql = "select " + rm.schema();
        sql += " where loan_id = ? and  g.is_active<>0  group by g.id";
        final List<JointBorrowerData> guarantorDatas = this.jdbcTemplate.query(sql, rm,
                new Object[] {loanId });

        final List<JointBorrowerData> mergedGuarantorDatas = new ArrayList<>();

        for (final JointBorrowerData guarantorData : guarantorDatas) {
    ...
    @Override
    public JointBorrowerData retrieveGuarantor(final Long loanId, final Long guarantorId) {
        final GuarantorMapper rm = new GuarantorMapper();
        String sql = "select " + rm.schema();
        sql += " where g.loan_id = ? and g.id = ? group by g.id";
        final JointBorrowerData guarantorData = this.jdbcTemplate.queryForObject(sql, rm,
    ...
    private static final class GuarantorMapper implements RowMapper<JointBorrowerData> {
        ...
        @Override
        public JointBorrowerData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            final EnumOptionData guarantorType = JointBorrowerEnumerations.guarantorType(guarantorTypeId);
          ...
          /*  List<GuarantorFundingData> guarantorFundingDetails = null;
            ...
            return new JointBorrowerData(id, loanId, clientRelationshipType, entityId, guarantorType, firstname, lastname, dob, addressLine1,
                    addressLine2, city, state, zip, country, mobileNumber, housePhoneNumber, comment, null, null, null, status,
                     null, null, accountLinkingOptions,clientAccountno);
        }
    }
    ...
    private static final class GuarantorFundingMapper implements RowMapper<GuarantorFundingData> {
        ...
        public GuarantorFundingData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            if (id != null && id > 0) {
                ...
                final EnumOptionData status = JointBorrowerEnumerations.guarantorFundStatusType(statusEnum);
     ...
     * @param guarantorData
     */
    private JointBorrowerData mergeDetailsForClientOrStaffGuarantor(final JointBorrowerData guarantorData) {
        if (guarantorData.isExistingClient()) {
            ...
            return JointBorrowerData.mergeClientData(clientData, guarantorData);
            ...
            return JointBorrowerData.mergeStaffData(staffData, guarantorData);
        }
