...
import org.apache.fineract.portfolio.loanaccount.data.LoanDueData;
...
public class LoanArrearsAgingServiceImpl implements LoanArrearsAgingService, BusinessEventListner {
    ...
    @Autowired
    public LoanArrearsAgingServiceImpl(final RoutingDataSource dataSource, final BusinessEventNotifierService businessEventNotifierService,final SmsProcessingService smsProcessingService) {
    ...
    public void updateLoanArrearsAgeingDetails() {
        ...
        //Job to send sms to remind repayments starts
        try {
        	String[] ary = env.getProperty("sms.repaymentReminder.days").split(",");
        	ArrayList<Integer> integerArr = new ArrayList<Integer>();  
        	for(String count:ary) {
        		integerArr.add(Integer.valueOf(count));
        	}
        	Set<Integer> ids = 
        	...
        	LoanRepaymentTemplateMapper mapper = new LoanRepaymentTemplateMapper();
            String sql = mapper.schema();
            boolean isActive = Boolean.valueOf(env.getProperty("sms.repaymentReminder.isActive"));
            if(isActive && CommonMethodsUtil.isNotBlank(ids)) {
            final Collection<LoanDueData> loanTransactionData = this.namedJdbctemplate.query(sql, parameters,mapper);
            if(CommonMethodsUtil.isNotBlank(loanTransactionData)) {	
            for(LoanDueData arrearData : loanTransactionData) {
            	if(CommonMethodsUtil.isNotBlank(arrearData.getLoanAccountNumber())&&CommonMethodsUtil.isNotBlank(arrearData.getDueAmount())) {
                    String templateParamString = "templateParameters[B]="+arrearData.getLoanAccountNumber()+"&templateParameters[A]="+arrearData.getDueAmount().setScale(2, BigDecimal.ROUND_HALF_EVEN).toString()+"&templateParameters[C]="+arrearData.getDueSinceDate().toString("dd/MM/yyyy");
                    SMSDataVO smsdata = new SMSDataVO("repaymentReminder", arrearData.getClientMobileNumber(), templateParamString);
        	...
        	logger.error(ThreadLocalContextUtil.getTenant().getName() + ": Exception in sending repayment reminder sms: " + e.getMessage());
        }
        //Job to send sms to remind repayments ends

        logger.info(ThreadLocalContextUtil.getTenant().getName() + ": Results affected by update: " + result);
    }

    @SuppressWarnings("unused")
	private static final class LoanRepaymentTemplateMapper implements RowMapper<LoanDueData> {


        public String schema() {
            StringBuilder sqlBuilder = new StringBuilder();

//            sqlBuilder.append("SELECT ifnull(cur.display_symbol, ml.currency_code) as currency,");
            sqlBuilder.append("SELECT  ml.currency_code as currency,");
            sqlBuilder.append("mc.account_no as client_account_number,");
            sqlBuilder.append("mc.display_name AS client_name,");
            sqlBuilder.append("mc.mobile_no as mobile_no,");
            sqlBuilder.append("ml.account_no AS loan_account_number,");
            sqlBuilder.append("lrs.duedate AS due_date,");
            sqlBuilder.append("ifnull( `getTotalAmountOfInstallmentOnDate`(lrs.loan_id,lrs.installment,lrs.duedate),0) AS due_amount ");
            sqlBuilder.append(" FROM m_loan_repayment_schedule lrs ");
            sqlBuilder.append(" INNER JOIN m_loan ml ON ml.id = lrs.loan_id ");
            sqlBuilder.append(" INNER JOIN m_client mc ON mc.id=ml.client_id ");
//            sqlBuilder.append(" left join m_currency cur on cur.code = ml.currency_code ");
            sqlBuilder.append(" WHERE lrs.completed_derived = 0 and ml.loan_status_id=300 ");
            sqlBuilder.append(" and mc.mobile_no IS NOT NULL ");
            sqlBuilder.append(" and DATEDIFF(lrs.duedate,CURDATE()) in (:ids) ");
            sqlBuilder.append(" ORDER BY  ml.account_no ");
            return sqlBuilder.toString();

        }

        @Override
        public LoanDueData mapRow(ResultSet rs, int rowNum) throws SQLException {
                        
        
        	final String currency = rs.getString("currency");
            final String loanAccountNumber = rs.getString("loan_account_number");
            final String clientAccountNumber = rs.getString("client_account_number");
            final String clientName = rs.getString("client_name");
            final String clientMobileNumber = rs.getString("mobile_no");
            final BigDecimal dueAmount = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "due_amount");
            final LocalDate date = JdbcSupport.getLocalDate(rs, "due_date");
            
           
            return new LoanDueData(currency,loanAccountNumber,clientAccountNumber,clientName,clientMobileNumber,dueAmount,date);
        }
}
