...
public class HolidaysApiResource {
    ...
    @GET
    @Path("/allActiveHolidays")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String retrieveAllActiveHolidays(@Context final UriInfo uriInfo, @QueryParam("officeId") final Long officeId,
            @QueryParam("fromDate") final DateParam fromDateParam, @QueryParam("toDate") final DateParam toDateParam,
            @QueryParam("locale") final String locale, @QueryParam("dateFormat") final String dateFormat) {
        ...
        final Collection<HolidayData> holidays = this.holidayReadPlatformService.retrieveAllActiveHolidaysBySearchParamerters(officeId, fromDate,
                toDate);
