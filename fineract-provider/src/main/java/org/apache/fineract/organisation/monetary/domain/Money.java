...
public class Money implements Comparable<Money> {
    ...
    public static Money of1(final MonetaryCurrency currency, final BigDecimal newAmount,final String istrue) {
        return new Money(currency.getCode(), currency.getDigitsAfterDecimal(), defaultToZeroIfNull(newAmount),
                currency.getCurrencyInMultiplesOf(),istrue);
    }
    ...
    // Habile Arivu new config
    private Money(final String currencyCode, final int digitsAfterDecimal, final BigDecimal amount, final Integer inMultiplesOf,final String istrue) {
        this.currencyCode = currencyCode;
        this.currencyDigitsAfterDecimal = digitsAfterDecimal;
        this.inMultiplesOf = inMultiplesOf;
