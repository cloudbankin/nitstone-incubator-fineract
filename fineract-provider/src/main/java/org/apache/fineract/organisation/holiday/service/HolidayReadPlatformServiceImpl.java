...
public class HolidayReadPlatformServiceImpl implements HolidayReadPlatformService {
    ...
    @Override
    public Collection<HolidayData> retrieveAllActiveHolidaysBySearchParamerters(final Long officeId, final Date fromDate, final Date toDate) {
        this.context.authenticatedUser();
        ...
        final HolidayMapper rm = new HolidayMapper();
        String sql = "select " + rm.schema() + " join m_holiday_office hf on h.id = hf.holiday_id and h.status_enum = 300 ";
