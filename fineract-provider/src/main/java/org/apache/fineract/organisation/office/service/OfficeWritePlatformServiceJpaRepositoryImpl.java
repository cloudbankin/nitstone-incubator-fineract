...
public class OfficeWritePlatformServiceJpaRepositoryImpl implements OfficeWritePlatformService {
    ...
    public CommandProcessingResult createOffice(final JsonCommand command) {
        ...
        try {
            ...
            if (command.parameterExists("selectedHoliday")) {
            	holidayList = command.arrayValueOfParameterNamed("selectedHoliday");
            }
            
            if(holidayList!= null)
            {
	            for(String holidayId: holidayList)
	            {
	            	Holiday holiday = this.holidayRepository.findOneWithNotFoundDetection(Long.parseLong(holidayId));
	            	/*HolidayOffice office_holiday = new HolidayOffice();
	                office_holiday.setOffice(office);
	                office_holiday.setHoliday(holiday);
	                this.officeHolidayRepository.save(office_holiday);*/
	                
	            	Set<Office> offices = new HashSet<Office>();
	            	for(Office exsistingOffice : holiday.getOffices() )
	            	{
	            		offices.add(exsistingOffice);
	            	}
	            	offices.add(office);
